...
const { getDataGolfLiveMatchups } = require('../utilities/dataGolfClient');
...
const resolveLiveMatchupsPath = ({ tour, market }) => {
  ...
  return path.resolve(ODDS_LIVE_DIR, 'matchups', safeTour, safeMarket, 'latest.json');
};

const main = async () => {
  const args = parseArgs();
  const tour = String(args.tour || 'pga').trim().toLowerCase() || 'pga';
  const market = String(args.market || 'tournament_matchups').trim().toLowerCase() || 'tournament_matchups';
  ...
  const snapshot = await getDataGolfLiveMatchups({
    apiKey,
    cacheDir: CACHE_DIR,
    ttlMs,
    allowStale: true,
  ...
  if (!snapshot?.payload) {
    console.error('❌ Live matchups payload not available.');
    process.exit(1);
  }

  const outputPath = resolveLiveMatchupsPath({ tour, market });
  writeJson(outputPath, snapshot.payload);
  console.log(`✓ Live matchups saved to ${outputPath} (source: ${snapshot.source})`);
};
