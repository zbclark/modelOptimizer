...
const { getTournamentResultsFormattingSchema } = require('./tournamentResultsFormattingSchema');
...
const schema = getTournamentResultsFormattingSchema();

const writeJson = () => {
  const jsonPath = path.resolve(OUTPUT_DIR, 'tournament_results_formatting_schema.json');
  fs.writeFileSync(jsonPath, JSON.stringify(schema, null, 2));
  console.log(`✅ Wrote formatting JSON: ${jsonPath}`);
};

const writeCsv = () => {
  const csvPath = path.resolve(OUTPUT_DIR, 'tournament_results_formatting_schema.csv');
  const lines = [];
