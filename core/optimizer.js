...
const {
  ...
  parseTimestampToMs
} = require('../utilities/timeUtils');
const { loadCsv } = require('../utilities/csvLoader');
const { ensureDirectory, readJsonFile, writeJsonFile } = require('../utilities/fileUtils');
...
const { parseEnvNumber } = require('../utilities/envParser');
const { parseArgs } = require('../utilities/argParser');
const { buildMetricGroupsFromConfig } = require('../utilities/metricConfigBuilder');
const { flattenMetricWeights } = require('../utilities/metricWeights');
...
const { OutputArtifactManager } = require('../utilities/outputArtifactManager');
...
const APPROACH_DELTA_ROOT_DIR = path.resolve(ROOT_DIR, 'data', 'approach_deltas');
...
const resolveApproachDeltaDirsForSeason = season => {
  const dirs = [];
  if (season) {
    dirs.push(path.resolve(APPROACH_DELTA_ROOT_DIR, String(season)));
  }
  dirs.push(APPROACH_DELTA_ROOT_DIR);
  return dirs;
};

const resolveApproachDeltaOutputDir = season => {
  if (!season) return APPROACH_DELTA_ROOT_DIR;
  return path.resolve(APPROACH_DELTA_ROOT_DIR, String(season));
};

const buildApproachDeltaSearchDirs = (season, extraDirs = []) => {
  const dirs = [...resolveApproachDeltaDirsForSeason(season), ...(extraDirs || [])];
...
const buildFeatureVector = (player, metricSpecs) => {
  ...
  const features = specs.map(spec => {
    const { label, index, valueGetter } = spec;
    const rawValue = typeof valueGetter === 'function'
      ? valueGetter(player)
      : (typeof index === 'number' ? player.metrics[index] : null);
...
let FORCE_FIELD_UPDATES = false;
let RUN_VALIDATION_ONLY = false;
let RUN_VALIDATION_REQUESTED = false;
let RUN_RESULTS_ONLY = false;
let RUN_DELTA_ONLY = false;

if (parsedArgs.hasFlag(['--log', '--verbose'])) {
  LOGGING_ENABLED = true;
}
if (parsedArgs.hasFlag(['--dryRun', '--dry-run'])) {
  DRY_RUN = true;
}
...
for (let i = 0; i < args.length; i++) {
  ...
  if (
    args[i] === '--forceFieldUpdates'
    || args[i] === '--force-field-updates'
    || args[i] === '--refreshFieldUpdates'
    || args[i] === '--refresh-field-updates'
  ) {
    FORCE_FIELD_UPDATES = true;
  }
...
const buildRunContext = ({
  ...
  runDeltaOnly,
  tournamentName,
  postTournamentHint = null
} = {}) => {
  const normalizedTournamentName = String(tournamentName || '').trim();
  const normalizedTournamentNameLower = normalizedTournamentName.toLowerCase();
  const wantsValidationOnly = runValidationOnly && !forcePostFlag && !forcePreFlag;
  const isSeasonValidationName = normalizedTournamentNameLower === 'all';
  const isSeasonValidation = !!wantsValidationOnly && (
    !normalizedTournamentName
    || ['all', 'season', 'all-tournaments', '*'].includes(normalizedTournamentNameLower)
  );

  let mode = null;
  if (wantsValidationOnly) mode = RUN_MODES.VALIDATION_ONLY;
  else if (runDeltaOnly) mode = RUN_MODES.DELTA_ONLY;
  else if (runResultsOnly) mode = RUN_MODES.RESULTS_ONLY;
  else if (forceMode === 'pre') mode = RUN_MODES.PRE;
  else if (forceMode === 'post') mode = RUN_MODES.POST;
  else if (postTournamentHint === true) mode = RUN_MODES.POST;
  else if (postTournamentHint === false) mode = RUN_MODES.PRE;

  const isPre = mode === RUN_MODES.PRE;
  const isPost = mode === RUN_MODES.POST
    || mode === RUN_MODES.RESULTS_ONLY
    || mode === RUN_MODES.VALIDATION_ONLY;
  const isResultsOnly = mode === RUN_MODES.RESULTS_ONLY;
  const isValidationOnly = mode === RUN_MODES.VALIDATION_ONLY;
  const isDeltaOnly = mode === RUN_MODES.DELTA_ONLY;
  const shouldScaffold = !isResultsOnly && !isSeasonValidation;
  const requireTournamentName = forceMode === 'pre'
    || forceMode === 'post'
    || runResultsOnly
    || (validationRequested && !wantsValidationOnly)
    || (!forceMode && !postTournamentHint);
  const label = forcePreFlag
    ? 'pre_event'
    : (forcePostFlag ? 'post_event' : 'run');

  return {
    mode,
    label,
    forceMode,
    forcePreFlag: !!forcePreFlag,
    forcePostFlag: !!forcePostFlag,
    postTournamentHint,
    normalizedTournamentName,
    isSeasonValidationName,
    isSeasonValidation,
    isPre,
    isPost,
    isResultsOnly,
    ...
    shouldScaffold,
    requireTournamentName,
    preferPostOutputDir: !!isPost || (!!postTournamentHint && !forceMode)
  };
};
...
const runContextBase = buildRunContext({
  forceMode: FORCE_RUN_MODE,
  forcePreFlag: FORCE_PRE_FLAG,
  forcePostFlag: FORCE_POST_FLAG,
  runResultsOnly: RUN_RESULTS_ONLY,
  runValidationOnly: RUN_VALIDATION_ONLY,
  validationRequested: RUN_VALIDATION_REQUESTED,
  runDeltaOnly: RUN_DELTA_ONLY,
  tournamentName: TOURNAMENT_NAME
});

if (!OVERRIDE_EVENT_ID && !runContextBase.isSeasonValidation) {
...
function buildHistoricalCacheSuffix({ tour = 'pga', eventId = 'all', year }) {
  const safeTour = String(tour || 'pga').trim().toLowerCase() || 'pga';
  const safeEventId = String(eventId ?? 'all').trim().toLowerCase() || 'all';
  const safeYear = String(year || '').trim();
  return `${safeTour}_${safeEventId}_${safeYear || 'year'}`
    .replace(/[^a-z0-9._-]/g, '_');
}

function buildHistoricalCachePaths({ cacheDir, tours = [], years = [], eventId = 'all', fileFormat = 'json' }) {
  const safeEventId = String(eventId ?? 'all').trim().toLowerCase() || 'all';
  const cachePaths = [];
  (tours || []).forEach(tour => {
    const safeTour = String(tour || 'pga').trim().toLowerCase() || 'pga';
    (years || []).forEach(year => {
      const cachePath = resolveHistoricalCachePath({
        cacheDir,
        tour: safeTour,
        year,
        eventId: safeEventId,
      ...
      if (cachePath && fs.existsSync(cachePath)) cachePaths.push(cachePath);
    });
  });
  return cachePaths;
}

function buildHistoricalCachePath({ cacheDir, tour = 'pga', year, eventId = 'all' }) {
  if (!cacheDir) return null;
  const cacheSuffix = buildHistoricalCacheSuffix({ tour, eventId, year });
  return path.resolve(cacheDir, `datagolf_historical_rounds_${cacheSuffix}.json`);
}

function resolveHistoricalCachePath({ cacheDir, tour = 'pga', year, eventId = 'all' }) {
  if (!cacheDir) return null;
  const primaryPath = buildHistoricalCachePath({ cacheDir, tour, year, eventId });
  if (primaryPath && fs.existsSync(primaryPath)) return primaryPath;

  const safeTour = String(tour || 'pga').trim().toLowerCase() || 'pga';
  const safeEventId = String(eventId ?? 'all').trim().toLowerCase() || 'all';
  const safeYear = String(year || '').trim();
  const legacySuffix = `${safeTour}_${safeEventId}_${safeYear || 'year'}`
    .replace(/[^a-z0-9._-]/g, '_');
  const legacyPath = path.resolve(cacheDir, `datagolf_historical_rounds_${legacySuffix}.json`);
  if (legacyPath && fs.existsSync(legacyPath)) {
    if (primaryPath && !fs.existsSync(primaryPath)) {
      try {
        fs.renameSync(legacyPath, primaryPath);
        console.log(`ℹ️  Historical cache renamed: ${path.basename(legacyPath)} → ${path.basename(primaryPath)}`);
        return primaryPath;
      } catch (error) {
        console.warn(`⚠️  Unable to rename historical cache ${path.basename(legacyPath)}: ${error.message}`);
        return legacyPath;
      }
    }
    return legacyPath;
  ...
  const legacyWithFormatSuffix = `${safeTour}_${safeEventId}_${safeYear || 'year'}_json`
    .replace(/[^a-z0-9._-]/g, '_');
  const legacyWithFormatPath = path.resolve(cacheDir, `datagolf_historical_rounds_${legacyWithFormatSuffix}.json`);
  if (legacyWithFormatPath && fs.existsSync(legacyWithFormatPath)) {
    if (primaryPath && !fs.existsSync(primaryPath)) {
      try {
        fs.renameSync(legacyWithFormatPath, primaryPath);
        console.log(`ℹ️  Historical cache renamed: ${path.basename(legacyWithFormatPath)} → ${path.basename(primaryPath)}`);
        return primaryPath;
      } catch (error) {
        console.warn(`⚠️  Unable to rename historical cache ${path.basename(legacyWithFormatPath)}: ${error.message}`);
        return legacyWithFormatPath;
      }
    }
    return legacyWithFormatPath;
  }
...
function extractEventHistoricalRows({ payload, eventId, season }) {
  const eventIdStr = String(eventId || '').trim();
  const seasonStr = String(season || '').trim();
  if (!payload || !eventIdStr || !seasonStr) return [];
  return extractHistoricalRowsFromSnapshotPayload(payload)
    .map(normalizeHistoricalRoundRow)
    .filter(Boolean)
    .filter(row => {
      const rowEvent = String(row['event_id'] || '').trim();
      if (rowEvent !== eventIdStr) return false;
      const rowSeason = String(row['season'] || row['year'] || '').trim();
      return rowSeason === seasonStr;
    });
}
...
function getEnvNumberValue(name, fallback) {
  const parsed = parseEnvNumber(name, fallback);
...
function findApproachDeltaFile(dirs, tournamentName, fallbackName, season = null) {
  ...
  if (normalized) {
    ...
    if (matches.length > 0) {
      const seasonMatches = seasonPattern ? matches.filter(match => seasonPattern.test(match.file)) : [];
      const rankedMatches = seasonMatches.length > 0 ? seasonMatches : matches;
...
function resolveApproachSnapshotSourcePath(source) {
  ...
  if (!path.isAbsolute(source)) {
    const archiveCandidate = path.resolve(APPROACH_SNAPSHOT_ARCHIVE_DIR, source);
...
function buildRollingApproachDeltaRows(entries, metricSpecs, fieldIdSet, maxFiles = APPROACH_DELTA_ROLLING_EVENTS) {
  ...
  accum.forEach(entry => {
    const outputRow = {
      ...
      tournament_field: fieldIdSet ? fieldIdSet.has(entry.dg_id) : null
    };
    metricSpecs.forEach(spec => {
...
function blendApproachDeltaTrendScores({ rollingScores, priorScores, rollingWeight = 0.8, priorWeight = 0.2 }) {
  const blended = new Map();
  const addScores = (scores, weight) => {
    if (!Array.isArray(scores) || weight <= 0) return;
    scores.forEach(entry => {
      const dgId = entry?.dgId || entry?.dg_id || null;
      if (!dgId) return;
      if (!blended.has(dgId)) {
        blended.set(dgId, {
          dgId,
          playerName: entry.playerName || entry.player_name || null,
          scoreSum: 0,
          weightSum: 0,
          bucketScoreSums: {},
          bucketWeightSums: {},
      ...
      const target = blended.get(dgId);
      if (!target.playerName && (entry.playerName || entry.player_name)) {
        target.playerName = entry.playerName || entry.player_name;
      }
      if (typeof entry.score === 'number' && Number.isFinite(entry.score)) {
        target.scoreSum += entry.score * weight;
        target.weightSum += weight;
      }
      if (typeof entry.usedMetrics === 'number' && Number.isFinite(entry.usedMetrics)) {
        target.usedMetricsSum += entry.usedMetrics * weight;
        target.usedMetricsWeight += weight;
      }
      const bucketScores = entry.bucketScores || {};
      Object.entries(bucketScores).forEach(([bucketKey, value]) => {
        if (typeof value !== 'number' || !Number.isFinite(value)) return;
        target.bucketScoreSums[bucketKey] = (target.bucketScoreSums[bucketKey] || 0) + (value * weight);
        target.bucketWeightSums[bucketKey] = (target.bucketWeightSums[bucketKey] || 0) + weight;
      });
    });
  };

  addScores(rollingScores, rollingWeight);
  addScores(priorScores, priorWeight);

  const results = [];
  blended.forEach(entry => {
    if (!entry.weightSum) return;
    const bucketScores = {};
    Object.keys(entry.bucketScoreSums).forEach(bucketKey => {
      const bucketWeight = entry.bucketWeightSums[bucketKey] || 0;
      if (bucketWeight > 0) {
        bucketScores[bucketKey] = entry.bucketScoreSums[bucketKey] / bucketWeight;
      }
    });
    results.push({
      dgId: entry.dgId,
      playerName: entry.playerName || null,
      score: entry.scoreSum / entry.weightSum,
      usedMetrics: entry.usedMetricsWeight > 0 ? entry.usedMetricsSum / entry.usedMetricsWeight : 0,
      bucketScores
    });
  });
...
const RAW_APPROACH_BUCKET_LABELS = new Set([
...
const APPROACH_BUCKET_LABELS = RAW_APPROACH_BUCKET_LABELS;
const APPROACH_ROUGH_BUCKET_LABELS = new Set([
  'Approach <150 Rough GIR', 'Approach <150 Rough SG', 'Approach <150 Rough Prox',
  'Approach >150 Rough GIR', 'Approach >150 Rough SG', 'Approach >150 Rough Prox'
]);
...
function generateSheetLikePlayerNotes(player, groups, groupStats) {
  ...
  const bucketSignalZ = player.bucketSignal && Number.isFinite(player.bucketSignal.zScore)
    ? player.bucketSignal.zScore
    : null;
  if (bucketSignalZ !== null) {
    const bucketArrow = bucketSignalZ >= 0 ? '↑' : '↓';
    if (hasPoorFitNote && (bucketArrow === '↑' || bucketSignalZ >= 0.75)) {
      notes.push('ℹ️ Recent bucket trend strong (short-term) despite poor baseline fit');
    } else if (hasStrongFitNote && (bucketArrow === '↓' || bucketSignalZ <= -0.75)) {
...
const RESULTS_SNAPSHOT_FIELDS = [
  ...
  { label: 'Fairway Proximity' },
  { label: 'Rough Proximity' },
  { label: 'SG BS', hasModel: false },
  { label: 'Scoring Average' },
  { label: 'Birdies or Better' },
  { label: 'Scrambling' },
  { label: 'Great Shots' },
  { label: 'Poor Shot Avoidance', alias: 'Poor Shots' }
];

const buildModelByIdFromRankingPlayers = (players = []) => {
  const map = new Map();
  (players || []).forEach(player => {
    ...
    map.set(dgId, {
      ...
      metrics: Array.isArray(player?.metrics) ? player.metrics : null
    });
  });
  return map;
};

const buildTournamentResultsSnapshotRows = ({
  results,
  modelById,
  actualMetricsById
}) => {
  ...
  (results || []).forEach(entry => {
    ...
    if (!dgId) return;
    const modelEntry = modelById?.get(dgId) || null;
    const playerName = entry?.playerName || modelEntry?.name || 'Unknown';
    const finishPosition = Number.isFinite(entry?.finishPosition) ? entry.finishPosition : null;
    const finishText = entry?.finishText || (finishPosition !== null ? String(finishPosition) : '');
    const row = {
      'Performance Analysis': '',
      'DG ID': dgId,
      'Player Name': playerName,
      'Model Rank': modelEntry?.rank ?? '',
      'Finish Position': finishText || '',
      'Score': entry?.score ?? ''
    };

    const actualMetrics = actualMetricsById?.get(dgId) || [];
    RESULTS_SNAPSHOT_FIELDS.forEach(field => {
      const labelKey = field.alias || field.label;
      const metricIndex = typeof field.index === 'number' ? field.index : labelIndexMap.get(labelKey);
      const actualValue = typeof metricIndex === 'number' ? actualMetrics[metricIndex] : null;
      row[field.label] = typeof actualValue === 'number' && Number.isFinite(actualValue) ? actualValue : '';
      if (field.hasModel !== false) {
        const modelMetrics = Array.isArray(modelEntry?.metrics) ? modelEntry.metrics : [];
        const modelValue = typeof metricIndex === 'number' ? modelMetrics[metricIndex] : null;
        row[`${field.label} - Model`] = typeof modelValue === 'number' && Number.isFinite(modelValue) ? modelValue : '';
      }
    });
  ...
  rows.sort((a, b) => {
    const posA = Number(a['Finish Position']) || 999;
    const posB = Number(b['Finish Position']) || 999;
    return posA - posB;
  });
...
const computeSnapshotMetricStats = results => {
  const buckets = {};
  (results || []).forEach(entry => {
    RESULTS_SNAPSHOT_FIELDS.forEach(field => {
      if (field.hasModel === false) return;
...
const buildSnapshotZScores = (rows, metricStats) => {
  return (rows || []).map(row => {
    const zScores = {};
...
function buildSheetLikeRankingCsv(ranking, groups) {
  ...
  // NOTE: Leave DG ID blank so downstream validation parsers skip this row.
...
function trainTopNLogisticModel(players, results, metricLabels, options = {}) {
  ...
  const specs = normalizeMetricSpecs(metricLabels);
  const featureCount = specs.length;
  const rows = [];
  const labels = [];
  ...
  players.forEach(player => {
    ...
    for (let i = 0; i < featureCount; i++) {
      const spec = specs[i];
      const rawValue = typeof spec.valueGetter === 'function'
        ? spec.valueGetter(player)
        : (typeof spec.index === 'number' ? player.metrics[spec.index] : null);
      if (typeof rawValue !== 'number' || Number.isNaN(rawValue)) return;
      const adjustedValue = LOWER_BETTER_GENERATED_METRICS.has(spec.label) ? -rawValue : rawValue;
  ...
  const weightRanking = weights
    .map((weight, idx) => ({
      label: specs[idx]?.label || `Metric ${idx}`,
      weight,
...
function blendGroupSignalWeights({
  metricDerived,
  groupSignal,
  fallbackGroupWeights,
  maxAbsCorrelation,
  minCorr = 0.2,
  fullCorr = 0.35,
  maxShare = 0.6
}) {
  const metricWeights = buildFilledGroupWeights(metricDerived, fallbackGroupWeights);
  const groupWeights = buildFilledGroupWeights(groupSignal, fallbackGroupWeights);
  const effectiveMax = typeof maxAbsCorrelation === 'number' ? maxAbsCorrelation : 0;
  const normalized = effectiveMax <= minCorr
    ? 0
    : clamp01((effectiveMax - minCorr) / Math.max(0.001, fullCorr - minCorr));
  const groupShare = clamp01(maxShare * normalized);
  const metricShare = 1 - groupShare;
  const blended = blendGroupWeights(metricWeights, groupWeights, metricShare, groupShare);

  return {
    weights: groupWeightsMapToArray(blended),
    groupShare,
    metricShare,
    maxAbsCorrelation: effectiveMax,
    minCorr,
    fullCorr,
    maxShare
  };
...
function normalizeMetricSpecs(metricSpecs) {
  ...
  if (typeof metricSpecs[0] === 'string') {
    return metricSpecs
      .map(label => ({ label, index: GENERATED_METRIC_LABELS.indexOf(label), valueGetter: null }))
      .filter(spec => typeof spec.index === 'number' && spec.index >= 0);
  }
  return metricSpecs.map((spec, index) => ({
    label: spec.label,
    index: typeof spec.index === 'number' ? spec.index : index,
    valueGetter: typeof spec.valueGetter === 'function' ? spec.valueGetter : null
  }));
}
...
function computeGroupCorrelations(players, results, groups, topN = null) {
  if (!Array.isArray(players) || !Array.isArray(results) || !Array.isArray(groups)) return [];
  const { map: resultsById } = buildFinishPositionMap(results);
  ...
  return groups.map(group => {
    ...
    players.forEach(player => {
      ...
      const finishPosition = resultsById.get(dgId);
      if (typeof finishPosition !== 'number' || Number.isNaN(finishPosition)) return;
      const score = player?.groupScores?.[groupName];
      if (typeof score !== 'number' || Number.isNaN(score)) return;
      xValues.push(score);
      if (typeof topN === 'number') {
        yValues.push(finishPosition <= topN ? 1 : 0);
      } else {
        yValues.push(-finishPosition);
      }
    });
    ...
    if (xValues.length < 5) {
      return { groupName, correlation: 0, samples: xValues.length };
    }
...
function buildSuggestedGroupWeightsFromGroupSignals(groupCorrelations, sourceLabel = 'group-signal') {
  if (!Array.isArray(groupCorrelations) || groupCorrelations.length === 0) {
    return { source: sourceLabel, weights: [] };
  }
  const totals = groupCorrelations.reduce((acc, entry) => {
    if (!entry || !entry.groupName) return acc;
    const value = Math.abs(entry.correlation || 0);
    if (!Number.isFinite(value) || value <= 0) return acc;
    acc[entry.groupName] = (acc[entry.groupName] || 0) + value;
    return acc;
  }, {});
  const total = Object.values(totals).reduce((sum, value) => sum + value, 0);
  const weights = Object.entries(totals)
    .map(([groupName, value]) => ({ groupName, weight: total > 0 ? value / total : 0 }))
    .sort((a, b) => b.weight - a.weight);

  return { source: sourceLabel, weights };
}

function buildTopNSamplesFromPlayers(players, results, metricSpecs) {
  const { map: resultsById } = buildFinishPositionMap(results);
...
function computeCvReliability(cvSummary, options = {}) {
  ...
  const eventCount = typeof cvSummary.effectiveEventCount === 'number'
    ? cvSummary.effectiveEventCount
    : (typeof cvSummary.weightedEventCount === 'number'
      ? cvSummary.weightedEventCount
      : (typeof cvSummary.eventCount === 'number' ? cvSummary.eventCount : 0));
...
function getPayloadLastUpdated(payload) {
  if (!payload || typeof payload !== 'object') return null;
  return payload.last_updated || payload.lastUpdated || null;
}

function appendCentralLastUpdated(payload) {
  if (!payload || typeof payload !== 'object') return payload;
  if (payload.last_updated_ct) return payload;
  const lastUpdated = getPayloadLastUpdated(payload);
  if (!lastUpdated) return payload;
  const centralValue = buildCentralTimestampFromUtcString(lastUpdated);
  if (!centralValue) return payload;
  return { ...payload, last_updated_ct: centralValue };
}
...
function hasHistoricalResultsInCache({ cacheDir, tour, eventId, season }) {
  ...
  for (const cachePath of cachePaths) {
    ...
    const hasMatch = rows.some(row => {
      ...
      const rowSeason = String(row?.season || row?.year || '').trim();
      if (seasonKey && rowSeason !== seasonKey) return false;
...
function normalizeApproachSnapshotArchiveName({ period, filePath, dateStamp }) {
  if (!filePath || !fs.existsSync(filePath)) return { filePath, dateStamp };
  const payload = readJsonFile(filePath);
  const updatedRaw = getPayloadLastUpdated(payload);
  const updatedMs = parseTimestampToMs(updatedRaw);
  if (!updatedMs) return { filePath, dateStamp };
  const updatedStamp = formatDateKey(new Date(updatedMs));
  if (!updatedStamp || updatedStamp === dateStamp) return { filePath, dateStamp };

  const targetName = `approach_${period}_${updatedStamp}.json`;
  const targetPath = path.resolve(path.dirname(filePath), targetName);
  if (fs.existsSync(targetPath)) {
    console.warn(`⚠️  Snapshot archive name mismatch (${path.basename(filePath)}); target exists (${path.basename(targetPath)}).`);
    return { filePath: targetPath, dateStamp: updatedStamp };
  }

  try {
    fs.renameSync(filePath, targetPath);
    console.log(`ℹ️  Renamed approach snapshot: ${path.basename(filePath)} → ${path.basename(targetPath)}`);
    return { filePath: targetPath, dateStamp: updatedStamp };
  } catch (error) {
    console.warn(`⚠️  Unable to rename snapshot ${path.basename(filePath)}: ${error.message}`);
    return { filePath, dateStamp };
  }
}
...
function listApproachSnapshotArchives() {
  ...
  const collect = baseDir => {
    ...
    files.forEach(name => {
      ...
      let dateStamp = match[2];
      let filePath = path.resolve(baseDir, name);
      const normalized = normalizeApproachSnapshotArchiveName({ period, filePath, dateStamp });
      filePath = normalized.filePath;
      dateStamp = normalized.dateStamp;
...
function pruneApproachSnapshotArchives() {
  ...
  Object.entries(byPeriod).forEach(([period, entries]) => {
    ...
    sorted.forEach((entry, index) => {
      ...
      if (entry.baseDir && path.resolve(entry.baseDir) === path.resolve(APPROACH_SNAPSHOT_ARCHIVE_DIR)) {
        removed += 1;
        return;
      }
      const targetPath = path.resolve(APPROACH_SNAPSHOT_ARCHIVE_DIR, entry.name);
      try {
        if (!fs.existsSync(targetPath)) {
        ...
        console.warn(`⚠️  Unable to archive approach snapshot ${entry.name}: ${error.message}`);
      }
    });
  });
...
async function getOrCreateApproachSnapshot({ period, snapshotPath, apiKey, cacheDir, ttlMs, season, eventId, isPostTournament, cacheFirst = false }) {
  ...
  if (fetched?.payload) {
    const fetchedUpdatedRaw = getPayloadLastUpdated(fetched.payload);
    ...
    const archiveStamp = formatDateKey(archiveDate);

    if (normalizedPeriod === 'ytd') {
      let latestPayload = null;
      if (snapshotPath && fs.existsSync(snapshotPath)) {
        latestPayload = readJsonFile(snapshotPath);
      }
      ...
      const cacheLabel = latestUpdatedMs
        ? formatTimestamp(new Date(latestUpdatedMs))
        : (latestPayload ? 'missing' : 'none');
      const apiLabel = fetchedUpdatedMs
        ? formatTimestamp(new Date(fetchedUpdatedMs))
        : (fetchedUpdatedRaw || 'missing');
      console.log(`ℹ️  YTD snapshot compare: cache=${cacheLabel}, api=${apiLabel}, shouldWrite=${shouldWrite}`);
      if (!shouldWrite) {
        console.log('ℹ️  YTD approach snapshot already current; skipping write.');
        return { ...fetched, path: snapshotPath, payload: latestPayload };
      }

      const payloadToWrite = appendCentralLastUpdated(fetched.payload);
      if (snapshotPath) {
        writeJsonFile(snapshotPath, payloadToWrite);
      }
      const archivePath = path.resolve(APPROACH_SNAPSHOT_DIR, `approach_${normalizedPeriod}_${archiveStamp}.json`);
      if (!fs.existsSync(archivePath)) {
        writeJsonFile(archivePath, payloadToWrite);
      }
      pruneApproachSnapshotArchives();
      return { ...fetched, path: snapshotPath, payload: payloadToWrite };
    }
    ...
    return { ...fetched, path: archivePath, payload: payloadToWrite };
  }
...
async function refreshYtdApproachSnapshot({ apiKey, cacheDir, ttlMs, cacheFirst = false }) {
  const fetched = await getDataGolfApproachSkill({
    apiKey,
    cacheDir,
    ttlMs,
  ...
  if (fetched?.payload) {
    ...
    const payloadToWrite = appendCentralLastUpdated(fetched.payload);
    writeJsonFile(APPROACH_SNAPSHOT_YTD_LATEST_PATH, payloadToWrite);
    const archiveDate = fetchedUpdatedMs ? new Date(fetchedUpdatedMs) : new Date();
    const archiveStamp = formatDateKey(archiveDate);
    ...
    pruneApproachSnapshotArchives();
    return { ...fetched, path: APPROACH_SNAPSHOT_YTD_LATEST_PATH, archivePath, payload: payloadToWrite };
  }
...
function summarizeWeightChanges(fromWeights = {}, toWeights = {}, limit = 8) {
  const fromObj = fromWeights && typeof fromWeights === 'object' ? fromWeights : {};
  const toObj = toWeights && typeof toWeights === 'object' ? toWeights : {};
  const keys = new Set([...Object.keys(fromObj), ...Object.keys(toObj)]);
  const changes = [];

  keys.forEach(key => {
    const from = Number.isFinite(fromObj[key]) ? fromObj[key] : 0;
    const to = Number.isFinite(toObj[key]) ? toObj[key] : 0;
    const delta = to - from;
    if (from === 0 && to === 0) return;
    changes.push({ key, from, to, delta, absDelta: Math.abs(delta) });
  });
...
function summarizeMetricWeightChanges(fromWeights = {}, toWeights = {}, limit = 12) {
  const fromFlat = flattenMetricWeights(fromWeights);
  const toFlat = flattenMetricWeights(toWeights);
  return summarizeWeightChanges(fromFlat, toFlat, limit);
}
...
function formatRankingPlayers(players, groups = null, groupStats = null) {
  ...
  return players.map(player => {
    ...
    return {
      ...
      bucketSignal: player.bucketSignal
        ? {
          value: typeof player.bucketSignal.value === 'number' ? player.bucketSignal.value : null,
          zScore: typeof player.bucketSignal.zScore === 'number' ? player.bucketSignal.zScore : null,
          zRank: typeof player.bucketSignal.zRank === 'number' ? player.bucketSignal.zRank : null
        }
        : null,
      notes
    };
...
function deriveResultsFromLiveStats(payload) {
  const liveStats = Array.isArray(payload?.live_stats)
    ? payload.live_stats
  ...
  liveStats.forEach(row => {
    const dgId = String(row?.dg_id || row?.dgId || '').trim();
    if (!dgId) return;
    const rawPosition = row?.position ?? row?.fin_text ?? row?.finish ?? row?.finishPosition ?? null;
    const finishPosition = parseFinishPosition(rawPosition);
...
function resolveInputsAndDirs({
  ...
  const CURRENT_EVENT_ID = overrideEventId;
  const CURRENT_SEASON = overrideSeason ?? 2026;
  const shouldUpsertManifest = !!tournamentName;
  const manifestUpsert = shouldUpsertManifest
    ? ensureManifestEntry({
      ...
      tournamentName,
      tournamentSlug: normalizeTournamentSlug(tournamentName),
      logger: console
    })
    : null;
  const manifestEntries = manifestUpsert?.entries?.length
    ? manifestUpsert.entries
    : loadSeasonManifestEntries(dataRootDir, CURRENT_SEASON);
  const manifestEntry = resolveManifestEntryForEvent(manifestEntries, {
    eventId: CURRENT_EVENT_ID,
    tournamentSlug: normalizeTournamentSlug(tournamentName || tournamentNameFallback),
  ...
  const canonicalTournamentName = manifestTournamentName || tournamentName || tournamentNameFallback;
  const canonicalTournamentSlug = manifestTournamentSlug
    || normalizeTournamentSlug(tournamentName || tournamentNameFallback);
  ...
  const seasonDir = path.resolve(dataRootDir, String(CURRENT_SEASON));
  const tournamentDir = runContext.isSeasonValidation
    ? null
    : (manifestTournamentSlug
        ? path.resolve(seasonDir, manifestTournamentSlug)
        : resolveTournamentDir(
            CURRENT_SEASON,
            canonicalTournamentSlug || tournamentName,
            tournamentNameFallback
          ));
  const shouldScaffold = runContext.shouldScaffold;
  ...
  const preEventOutputDir = runContext.isSeasonValidation
    ? null
    : resolveModeRoot({
      tournamentRoot: tournamentDir,
      mode: 'pre_event',
      outputDirOverride: overrideOutputDir
    });
  const postEventOutputDir = runContext.isSeasonValidation
    ? null
    : resolveModeRoot({
      tournamentRoot: tournamentDir,
      mode: 'post_event',
  ...
  return {
    CURRENT_EVENT_ID,
    CURRENT_SEASON,
    shouldUpsertManifest,
    manifestUpsert,
    manifestEntries,
    manifestEntry,
    manifestTournamentSlug,
    manifestTournamentName,
...
function loadConfigAndTemplates({
  currentEventId,
  currentSeason,
  configPath,
  courseContextPath,
  ...
  let courseContext = courseContextInput || loadCourseContext(courseContextPath);
  if (!courseContext) {
    courseContext = loadCourseContext(courseContextPath);
  }
  ...
  if (courseContextEntry) {
    sharedConfig = buildSharedConfigFromCourseContext(courseContextEntry, currentEventId);
    console.log('ℹ️  Using course_context.json as primary configuration source.');
  }
  if (!sharedConfig && configPath && fs.existsSync(configPath)) {
    sharedConfig = getSharedConfig(configPath);
  ...
  const resolvedSeason = parseInt(sharedConfig.currentSeason || sharedConfig.currentYear || currentSeason);
  const effectiveSeason = Number.isNaN(resolvedSeason) ? currentSeason : resolvedSeason;
  ...
  const currentEventRoundsPolicy = includeCurrentEventRounds === null
    ? 'default'
    : (includeCurrentEventRounds ? 'include' : 'exclude');
  ...
  Object.entries(weightTemplates).forEach(([templateName, template]) => {
    if (!template) return;
  ...
  if (templateOverride) {
    if (!templateConfigs[templateOverride]) {
      console.error(`\n❌ Template not found or not available for event ${currentEventId}: ${templateOverride}`);
      console.error(`   Available: ${Object.keys(templateConfigs).join(', ') || 'none'}`);
      process.exit(1);
    }
    Object.keys(templateConfigs).forEach(name => {
      if (name !== templateOverride) delete templateConfigs[name];
    });
    console.log(`✓ Using template override: ${templateOverride}`);
  }

  if (!templateConfigs[currentEventId]) {
    ...
    if (fallbackTemplateKey) {
      templateConfigs[currentEventId] = {
      ...
      console.log(`✓ Loaded event-specific weights (eventId: ${currentEventId}) from template: ${fallbackTemplateKey}`);
    } else {
      const groupWeights = {};
      const metricWeights = {};
      baseMetricConfig.groups.forEach(group => {
      ...
      templateConfigs[currentEventId] = { groupWeights, metricWeights };
      console.warn(`⚠️  Loaded event-specific weights (eventId: ${currentEventId}) from base metric config (no templateKey/courseType found in course_context).`);
    }
  }
  ...
  return {
    ...
    sharedConfig,
    pastPerformanceWeightSummary,
    effectiveSeason,
    historicalYear,
    courseTemplateKey,
    currentEventRoundsPolicy,
    baseMetricConfig,
    templateConfigs,
    metricConfig
  };
...
async function prepareValidationOutputs({
  runContext,
  currentSeason,
  dataRootDir,
  tournamentName,
  ...
  const shouldAutoRunValidation = !runContext.isValidationOnly
    && !runContext.isResultsOnly
    && runContext.isPost;
  const normalizedValidationSlug = normalizeTournamentSlug(tournamentName || tournamentNameFallback) || baseName;
  const shouldRefreshValidationOutputs = (() => {
    if (!shouldAutoRunValidation || !validationOutputsDir) return false;
    const classificationPath = path.resolve(validationOutputsDir, `${outputNames.courseTypeClassification}.json`);
    ...
    const matchesEntry = entries.some(entry => {
      ...
      return (entryEventId && entryEventId === String(currentEventId || '').trim())
  ...
  if (shouldRefreshValidationOutputs) {
    try {
      const { runValidation } = require('../utilities/validationRunner');
  ...
  let validationData = loadValidationOutputs(metricConfig, [
    validationOutputsDir,
    path.resolve(dataRootDir, 'validation_outputs'),
    ...validationOutputDirs,
    DATA_DIR,
    DEFAULT_DATA_DIR
  ]);
  const skipValidationOutputs = runContext.isResultsOnly
  ...
  if (!validationData?.weightTemplatesPath || !validationData?.deltaTrendsPath) {
    ...
    if (!skipValidationOutputs && !runContext.isResultsOnly) {
      ...
      console.error(`   - ${path.resolve(dataRootDir, 'validation_outputs')}`);
...
async function prepareSnapshots({
  ...
  const IS_POST_TOURNAMENT_RUN = runContext.isPost;
  const IS_PRE_TOURNAMENT_RUN = runContext.isPre;
  const CACHE_FIRST_REFRESH = IS_POST_TOURNAMENT_RUN;
  const SHOULD_FETCH_APPROACH_ARCHIVES = (() => {
    ...
    const eventIdValue = String(currentEventId || '').trim();
  ...
  const hasApproachCsvFile = !!(approachPath && fs.existsSync(approachPath));
    ...
    if (!approachSnapshotL24?.payload) {
      try {
        approachSnapshotL24 = await getOrCreateApproachSnapshot({
          ...
          season: currentSeason,
          eventId: currentEventId,
          isPostTournament: runContext.postTournamentHint,
          cacheFirst: IS_POST_TOURNAMENT_RUN
        });
  ...
  if (shouldFetchApi) {
    ...
    try {
      const fieldTtlMs = IS_POST_TOURNAMENT_RUN
        ...
        : 0;
      const forceFieldUpdatesResolved = forceFieldUpdates
        || ['1', 'true', 'yes'].includes(String(process.env.FORCE_FIELD_UPDATES || '').trim().toLowerCase());
      ...
      fieldUpdatesSnapshot = await getDataGolfFieldUpdates({
        ...
        ttlMs: forceFieldUpdatesResolved ? 0 : fieldTtlMs,
        allowStale: forceFieldUpdatesResolved ? false : true,
        preferCache: forceFieldUpdatesResolved ? false : true,
      ...
      if (fieldUpdatesSnapshot?.payload?.event_name) {
        const forcedNote = forceFieldUpdatesResolved ? ', forced refresh' : '';
        console.log(`✓ DataGolf field updates loaded (${fieldUpdatesSnapshot.source}${forcedNote}, ${fieldUpdatesSnapshot.payload.event_name})`);
...
async function loadFieldAndWeather({
  fieldPath,
  fieldUpdatesSnapshot,
  currentEventId,
  tournamentName,
  ...
  console.log('\n🔄 Loading data...');
  const fieldCsvRows = (fieldPath && fs.existsSync(fieldPath))
    ? loadCsv(fieldPath, { skipFirstColumn: true })
    : [];
  ...
  if (fieldApiRows.length === 0 && fieldCsvRows.length > 0) {
    ...
    if (retainsFieldUpdatesPayload) {
      ...
      fieldUpdatesSnapshot = {
        ...
        path: fieldPath,
        payload: {
          event_id: currentEventId ? String(currentEventId).trim() : null,
          event_name: tournamentName || tournamentNameFallback || null,
          tour: datagolfFieldTour || null,
      ...
      try {
        ...
        if (weatherResult?.enabled) {
          ...
          const cachePayload = {
            ...
            tournamentName: canonicalTournamentName || tournamentName || tournamentNameFallback || null,
...
async function prepareHistoricalRounds({
  isPreTournamentRun,
  isPostTournamentRun,
  cacheFirstRefresh,
  effectiveSeason,
  ...
  const historyAnchorSeason = isPreTournamentRun ? (effectiveSeason - 1) : effectiveSeason;
  const historyYears = buildRecentYears(historyAnchorSeason, 5);
  if (isPreTournamentRun && !historyYears.includes(effectiveSeason)) {
  ...
  const contextEntry = resolveCourseContextEntry(context, currentEventId);
  ...
  const historyTtlMs = isPostTournamentRun
    ? 0
  ...
  const mergeEventYearRows = (rows, sourceLabel) => {
    if (!Array.isArray(rows) || rows.length === 0) return false;
    ...
    console.log(`✓ Loaded ${rows.length} rounds for event ${eventIdStr} (${seasonStr}) via ${sourceLabel}.`);
    return true;
  };
  ...
  if (historyEventSeasonCount === 0) {
    ...
    if (allEventsCachePath && fs.existsSync(allEventsCachePath)) {
      const cachedPayload = readJsonFile(allEventsCachePath);
      const eventYearRows = extractEventHistoricalRows({
        payload: cachedPayload,
        eventId: eventIdStr,
        season: seasonStr
      });
      if (eventYearRows.length > 0) {
        mergeEventYearRows(eventYearRows, 'all-events cache');
      }
    }
  }
  ...
  if (historyEventSeasonCount === 0 && !skipHistoricalApiRefresh) {
    try {
      if (!datagolfApiKey) {
        console.warn('ℹ️  Historical rounds API refresh skipped (missing DATAGOLF_API_KEY).');
      } else {
        const snapshot = await getDataGolfHistoricalRounds({
          apiKey: datagolfApiKey,
          cacheDir: null,
        ...
        if (eventYearRows.length > 0) {
          ...
          if (allEventsCachePath) {
            writeJsonFile(allEventsCachePath, snapshot?.payload || {});
            console.log(`ℹ️  Historical rounds cache updated for event ${eventIdStr} (${seasonStr}) in all-events cache.`);
          }
          mergeEventYearRows(eventYearRows, 'API payload');
        } else {
          console.warn(`ℹ️  Historical rounds missing for event ${eventIdStr} (${seasonStr}) after API check.`);
        }
      }
    } catch (error) {
    ...
    console.warn(`ℹ️  Skipping event-year historical rounds API fetch (refresh disabled; event ${eventIdStr}, season ${seasonStr}).`);
  }

  let liveResultsFallback = null;
  if (!isPreTournamentRun && historyEventSeasonCount === 0) {
    if (!datagolfApiKey) {
      console.warn('ℹ️  Live results fallback skipped (missing DATAGOLF_API_KEY).');
    } else {
      try {
        const liveSnapshot = await getDataGolfLiveTournamentStats({
          ...
          ttlMs: 0,
          allowStale: true,
          cacheFirst: cacheFirstRefresh
        });
        liveResultsFallback = deriveResultsFromLiveStats(liveSnapshot?.payload);
        if (liveResultsFallback.length > 0) {
          console.log(`✓ Loaded ${liveResultsFallback.length} live results from DataGolf (source: ${liveSnapshot?.source || 'unknown'}).`);
        } else {
          console.warn('ℹ️  Live tournament stats returned no results for fallback.');
        }
      } catch (error) {
        console.warn(`ℹ️  Live tournament stats fallback failed: ${error.message}`);
      }
    }
  }

  if (!isPreTournamentRun && historyEventSeasonCount === 0) {
    if (liveResultsFallback && liveResultsFallback.length > 0) {
      console.warn('⚠️  Historical rounds missing; proceeding with live results fallback.');
    } else {
...
async function runAdaptiveOptimizer() {
  ...
  const {
    ...
    postEventOutputDir,
    validationOutputsDir
  } = resolveInputsAndDirs({
    overrideEventId: OVERRIDE_EVENT_ID,
    overrideSeason: OVERRIDE_SEASON,
    tournamentName: TOURNAMENT_NAME,
    tournamentNameFallback,
    forceRunMode: FORCE_RUN_MODE,
    forcePreFlag: FORCE_PRE_FLAG,
    forcePostFlag: FORCE_POST_FLAG,
    runResultsOnly: RUN_RESULTS_ONLY,
    runValidationOnly: RUN_VALIDATION_ONLY,
    runDeltaOnly: RUN_DELTA_ONLY,
    overrideOutputDir: OVERRIDE_OUTPUT_DIR,
    dataRootDir: DATA_ROOT_DIR,
    rootDir: ROOT_DIR
  });
  let runContext = runContextBase;

  if (runContext.isValidationOnly) {
    try {
      const { runValidation, runSeasonValidation } = require('../utilities/validationRunner');
      if (runContext.isSeasonValidation && typeof runSeasonValidation === 'function') {
        console.log('🧪 Running validation pipeline for all season tournaments...');
      ...
      if (typeof runValidation !== 'function') {
        throw new Error('validation module is unavailable');
      }
      console.log('🧪 Running validation pipeline (standalone mode)...');
      ...
      if (typeof runSeasonValidation === 'function') {
        console.log('🧪 Running validation season rollup...');
        const seasonResult = await runSeasonValidation({
          season: CURRENT_SEASON,
          dataRootDir: DATA_ROOT_DIR,
          logger: console,
          writeTemplates: WRITE_TEMPLATES,
        ...
        if (seasonResult?.outputDir) {
          console.log(`✅ Season validation outputs written to: ${seasonResult.outputDir}`);
        }
      }
      return;
    } catch (error) {
  ...
  let APPROACH_DELTA_PATH = findApproachDeltaFile(
    buildApproachDeltaSearchDirs(CURRENT_SEASON, [OUTPUT_DIR, DATA_DIR, DEFAULT_DATA_DIR]),
    TOURNAMENT_NAME,
    tournamentNameFallback,
    CURRENT_SEASON
  );
  if (runContext.isDeltaOnly) {
  ...
  runContext = buildRunContext({
    ...
    runDeltaOnly: RUN_DELTA_ONLY,
    tournamentName: TOURNAMENT_NAME,
    postTournamentHint: POST_TOURNAMENT_HINT
  });
  if (runContext.requireTournamentName && !runContext.normalizedTournamentName) {
  ...
  if (runContext.isSeasonValidation && !runContext.isSeasonValidationName) {
  ...
  if (runContext.isValidationOnly && !runContext.isSeasonValidation && runContext.isSeasonValidationName) {
  ...
  if (LOGGING_ENABLED && !LOGGING_INITIALIZED) {
    ...
    if (loggingDir) {
      ...
      const kfoldTagSuffix = (runContext.label === 'post_event' && !OPT_SEED_RAW)
        ? `_${String(kfoldTag.tag || 'LOEO')}`
        : '';
      const logContext = `${runContext.label}${seedSuffix}${kfoldTagSuffix}${outputTagSuffix}`;
      ...
      const envKeysForLog = [
        ...
        'OPT_TESTS',
        'WRITE_TEMPLATES',
        'WRITE_VALIDATION_TEMPLATES',
        'APPLY_VALIDATION_OUTPUTS',
        'EVENT_KFOLD_K',
        'EVENT_KFOLD_SEED',
        'VALIDATION_APPROACH_MODE',
        'APPROACH_BLEND_WEIGHTS',
        'APPROACH_DELTA_ROLLING_EVENTS',
        'APPROACH_DELTA_MIN_DAYS',
        'APPROACH_DELTA_SNAPSHOT_ONLY_EVENTS',
        'MODEL_APPROACH_DELTA_TREND_ROLLING_WEIGHT',
        'MODEL_APPROACH_DELTA_TREND_PRIOR_WEIGHT',
        'MODEL_APPROACH_CAP_MIN',
        'MODEL_APPROACH_CAP_MAX',
        'DATAGOLF_APPROACH_PERIOD',
        'WEATHER_ENABLED',
        'WEATHER_BASE_LATE_EARLY_ADV',
        'MODEL_FIELD_MIN_PLAYERS',
        'MODEL_FIELD_MIN_APPROACH_OVERLAP'
      ];
      const envPairs = envKeysForLog
        .map(key => [key, process.env[key]])
        .filter(([, value]) => value !== undefined && value !== null && String(value).trim() !== '')
        .map(([key, value]) => `${key}=${String(value).trim()}`);
      if (envPairs.length > 0) {
        console.log(`Env: ${envPairs.join(', ')}`);
      }
  ...
  const SHOULD_RUN_PRE_EVENT_ARTIFACTS = runContext.isPre;
  ...
  const regressionSnapshot = SHOULD_RUN_PRE_EVENT_ARTIFACTS
    ...
    : null;
  let courseContextInitial = loadCourseContext(COURSE_CONTEXT_PATH);
  let courseContextUpdateSummary = null;
  if (courseContextInitial && regressionSnapshot?.map) {
    const upsertResult = upsertCourseContextPastPerformanceWeights(courseContextInitial, regressionSnapshot.map);
    courseContextUpdateSummary = {
      source: regressionSnapshot.source || null,
      path: regressionSnapshot.path || null,
    ...
    if (upsertResult.updated) {
      writeJsonFile(COURSE_CONTEXT_PATH, courseContextInitial);
  ...
  const configAndTemplates = loadConfigAndTemplates({
    currentEventId: CURRENT_EVENT_ID,
    currentSeason: CURRENT_SEASON,
    tournamentName: TOURNAMENT_NAME,
    tournamentNameFallback,
    ...
    courseContextPath: COURSE_CONTEXT_PATH,
    courseContextInput: courseContextInitial,
    regressionSnapshot,
    courseContextUpdateSummary,
    currentEventRoundsDefaults,
  ...
  let courseTemplateKey = configAndTemplates.courseTemplateKey;

  const baseName = normalizeTournamentSlug(TOURNAMENT_NAME || tournamentNameFallback)
    || `event-${CURRENT_EVENT_ID}`;

  const { validationData } = await prepareValidationOutputs({
    runContext,
    currentSeason: CURRENT_SEASON,
    dataRootDir: DATA_ROOT_DIR,
    tournamentName: TOURNAMENT_NAME,
    ...
    baseName,
    validationOutputsDir,
    validationOutputDirs: VALIDATION_OUTPUT_DIRS,
  ...
  let {
    ...
    shouldFetchApi,
    shouldFetchOptionalApi,
    forceFieldUpdates: FORCE_FIELD_UPDATES
  });
  ...
  if (fieldUpdatesMismatch && runContext.isPre) {
  ...
  const { fieldData, fieldUpdatesSnapshot: resolvedFieldUpdatesSnapshot, weatherWaveSummary } = await loadFieldAndWeather({
    fieldPath: FIELD_PATH,
    fieldUpdatesSnapshot,
    currentEventId: CURRENT_EVENT_ID,
    tournamentName: TOURNAMENT_NAME,
    tournamentNameFallback,
    datagolfFieldTour: DATAGOLF_FIELD_TOUR,
    preEventOutputDir,
    runContext,
    courseContextEntryFinal,
    effectiveSeason,
  ...
  fieldUpdatesSnapshot = resolvedFieldUpdatesSnapshot;
  ...
  const {
    ...
    tours,
    liveResultsFallback
  } = await prepareHistoricalRounds({
    isPreTournamentRun: IS_PRE_TOURNAMENT_RUN,
    isPostTournamentRun: IS_POST_TOURNAMENT_RUN,
    cacheFirstRefresh: CACHE_FIRST_REFRESH,
    effectiveSeason,
    currentEventId: CURRENT_EVENT_ID,
    tournamentName: TOURNAMENT_NAME,
    tournamentNameFallback,
  ...
  if (!hasApproachCsv && approachSnapshotRows.ytd.length === 0) {
    if (approachSnapshotRows.l12.length > 0) {
      approachDataCurrent = approachSnapshotRows.l12;
      approachDataCurrentSource = 'snapshot_l12';
    } else if (approachSnapshotRows.l24.length > 0) {
      approachDataCurrent = approachSnapshotRows.l24;
      approachDataCurrentSource = 'snapshot_l24';
    }
  }
  ...
  console.log(`   - Step 1c (event+similar+putting correlations): ${approachDataCurrentSource}`);
  if (!hasApproachCsv && approachBlendWeights) {
  ...
  if (resultsCurrent.length === 0 && liveResultsFallback && liveResultsFallback.length > 0) {
    resultsCurrent = liveResultsFallback;
    console.log(`ℹ️  Derived ${resultsCurrent.length} results from live tournament stats fallback.`);
  } else {
  ...
  if (!FORCE_PRE_FLAG && !FORCE_POST_FLAG) {
    console.error('\n❌ Missing required mode flag.');
    console.error('   Please rerun with an explicit mode flag: --post (recommended when results exist) or --pre.');
    process.exit(1);
  }
  if (runContext.forceMode === 'post' && !HAS_CURRENT_RESULTS) {
  ...
  if (runContext.isResultsOnly) {
    ...
    let rankingJsonPath = findFirstExisting(rankingJsonCandidates);
    if (!rankingJsonPath && preEventOutputDir && fs.existsSync(preEventOutputDir)) {
      const fallbackCandidates = fs.readdirSync(preEventOutputDir)
        .filter(name => name.toLowerCase().endsWith('_pre_event_rankings.json'))
        .map(name => path.resolve(preEventOutputDir, name));
      fallbackCandidates.sort((a, b) => a.localeCompare(b));
      rankingJsonPath = fallbackCandidates[0] || null;
      if (rankingJsonPath) {
        console.log(`ℹ️  Results-only: using fallback pre-event rankings JSON (${path.basename(rankingJsonPath)}).`);
      }
    }
    if (!rankingJsonPath) {
    ...
    let rankingCsvPath = findFirstExisting(rankingCsvCandidates);
    if (!rankingCsvPath && preEventOutputDir && fs.existsSync(preEventOutputDir)) {
      const fallbackCsvCandidates = fs.readdirSync(preEventOutputDir)
        .filter(name => name.toLowerCase().endsWith('_pre_event_rankings.csv'))
        .map(name => path.resolve(preEventOutputDir, name));
      fallbackCsvCandidates.sort((a, b) => a.localeCompare(b));
      rankingCsvPath = fallbackCsvCandidates[0] || null;
      if (rankingCsvPath) {
        console.log(`ℹ️  Results-only: using fallback pre-event rankings CSV (${path.basename(rankingCsvPath)}).`);
      }
    }
    ...
    if (HAS_CURRENT_RESULTS) {
      ...
      if (currentManifestEntry?.tournamentName || currentManifestEntry?.tournamentSlug) {
        const currentDeltaPath = findApproachDeltaFile(
          buildApproachDeltaSearchDirs(effectiveSeason, [OUTPUT_DIR, DATA_DIR, DEFAULT_DATA_DIR]),
      ...
      if (!approachDeltaPathFallback || shouldRecomputeResultsDelta) {
        ...
        if (previousRows.length === 0 || currentRows.length === 0) {
          ...
          if (taggedRows.length === 0) {
            ...
            const outputDir = resolveApproachDeltaOutputDir(CURRENT_SEASON);
    ...
    const resultsArtifacts = new OutputArtifactManager({
      outputBaseName,
      tournamentSlug: canonicalTournamentSlug || normalizeTournamentSlug(TOURNAMENT_NAME || tournamentNameFallback),
      tournamentName: canonicalTournamentName || TOURNAMENT_NAME || tournamentNameFallback,
      modeRoot: postEventOutputDir || OUTPUT_DIR
    });
    const resultsCsvPath = resultsArtifacts.buildPath({
    ...
    const resultsSnapshotRows = buildTournamentResultsSnapshotRows({
      results: resultsCurrent,
      modelById: rankingsById,
      actualMetricsById
    });
    const resultsSnapshotMetricStats = computeSnapshotMetricStats(resultsSnapshotRows);
    const resultsSnapshotZScores = buildSnapshotZScores(resultsSnapshotRows, resultsSnapshotMetricStats);
    ...
    const resultsJsonPayload = {
      ...
      results: resultsSnapshotRows,
      metricStats: resultsSnapshotMetricStats,
      zScores: resultsSnapshotZScores,
      resultsSheetCsv: resultsCsv,
      rankings: rankingPlayers,
      resultsRaw: resultsCurrent,
  ...
  let approachDeltaRows = approachDeltaRowsAll.filter(row => !!row);
  ...
  if (HAS_CURRENT_RESULTS && manifestEntries && manifestEntry) {
    ...
    if (approachDeltaPriorPath) {
      ...
      const priorRows = priorRowsAll.filter(row => !!row);
  ...
  let approachDeltaRollingRows = null;
  let approachDeltaRollingMeta = null;
  let approachDeltaRollingFiles = [];
  let approachDeltaPriorBlendRows = null;
  let approachDeltaPriorBlendPath = null;
  let approachDeltaTrendBlendSummary = null;
  ...
  if (HAS_CURRENT_RESULTS && (approachDeltaPriorRows || approachDeltaRows).length > 0) {
    ...
    if (IS_PRE_TOURNAMENT_RUN) {
      ...
      const currentManifestEntry = resolveManifestEntryForEvent(manifestEntriesForDelta, {
        eventId: CURRENT_EVENT_ID,
        tournamentSlug: normalizeTournamentSlug(canonicalTournamentSlug || TOURNAMENT_NAME || tournamentNameFallback),
        tournamentName: canonicalTournamentName || TOURNAMENT_NAME || tournamentNameFallback
      });
      const previousManifestEntry = currentManifestEntry
      ...
      if (priorDeltaPath) {
        ...
        const filteredRows = priorRowsAll.filter(row => !!row);
        if (filteredRows.length > 0) {
          priorDeltaRows = filteredRows;
          priorDeltaMeta = priorDeltaData?.meta || null;
          approachDeltaPriorBlendRows = filteredRows;
          approachDeltaPriorBlendPath = priorDeltaPath;
        }
      }
    }
    ...
    if (IS_PRE_TOURNAMENT_RUN && rollingResult?.meta?.filesUsed?.length) {
      ...
      const previousManifestEntry = currentManifestEntry
        ...
        : null;
      if (previousManifestEntry?.tournamentName || previousManifestEntry?.tournamentSlug) {
        const priorPathCheck = findApproachDeltaFile(
          buildApproachDeltaSearchDirs(CURRENT_SEASON, [OUTPUT_DIR, DATA_DIR, DEFAULT_DATA_DIR]),
          previousManifestEntry.tournamentName,
          previousManifestEntry.tournamentSlug,
          CURRENT_SEASON
        );
        if (priorPathCheck && !rollingResult.meta.filesUsed.includes(priorPathCheck)) {
    ...
    if (rollingResult.rows.length > 0) {
      approachDeltaRollingRows = rollingResult.rows;
      approachDeltaRollingMeta = rollingResult.meta || null;
      approachDeltaRollingFiles = rollingResult.meta?.filesUsed || [];
    }

    if (approachDeltaRollingRows && approachDeltaRollingRows.length > 0) {
      approachDeltaRowsAll = approachDeltaRollingRows;
      approachDeltaRows = approachDeltaRollingRows;
      approachDeltaAlignmentMap = buildApproachDeltaAlignmentFromRollingRows(
        approachDeltaMetricSpecs,
        approachDeltaRows
      );
      approachDeltaPriorMode = priorDeltaRows ? 'rolling_blend' : 'rolling_average';
      approachDeltaPriorMeta = approachDeltaRollingMeta || null;
      approachDeltaPriorFiles = priorDeltaRows && priorDeltaPath
        ? [...approachDeltaRollingFiles, priorDeltaPath]
        : approachDeltaRollingFiles;
      console.log(`✓ Built rolling approach delta baseline (${approachDeltaRollingMeta?.fileCount || 0} files).`);
    } else if (priorDeltaRows) {
      approachDeltaRowsAll = priorDeltaRows;
      approachDeltaRows = priorDeltaRows;
      approachDeltaAlignmentMap = buildApproachDeltaAlignmentFromRollingRows(
        approachDeltaMetricSpecs,
        approachDeltaRows
      );
      ...
      approachDeltaPriorFiles = priorDeltaPath ? [priorDeltaPath] : [];
  ...
  const runRanking = ({ roundsRawData, approachRawData, groupWeights, metricWeights, includeCurrentEventRounds = false, fieldDataOverride = null, useDeltaScores = true, deltaScoresOverride = null }) => {
    const adjustedMetricWeights = applyShotDistributionToMetricWeights(
      metricWeights,
      sharedConfig.courseSetupWeights
    );
    let deltaScoresByIdForRun = useDeltaScores ? runtimeConfig.deltaScoresById : {};
    if (useDeltaScores && deltaScoresOverride && Object.keys(deltaScoresOverride).length > 0) {
      deltaScoresByIdForRun = deltaScoresOverride;
    } else if (useDeltaScores && approachDeltaAlignmentMap.size > 0 && approachDeltaRows.length > 0) {
  ...
  const buildEventSamples = (eventRows, metricSpecsOverride = null) => {
    ...
    return ranking.players.reduce((acc, player) => {
      ...
      const metricSpecs = Array.isArray(metricSpecsOverride) && metricSpecsOverride.length > 0
        ? metricSpecsOverride
        : GENERATED_METRIC_LABELS.map((label, index) => ({ label, index }));
      const featurePack = buildFeatureVector(player, metricSpecs);
      if (!featurePack) return acc;
      acc.push({ features: featurePack.features, label: finishPosition <= 20 ? 1 : 0 });
      return acc;
  ...
  const buildEventSamplesBySeason = (season, metricSpecsOverride = null) => {
    ...
    eventMap.forEach((rows, eventId) => {
      const samples = buildEventSamples(rows, metricSpecsOverride);
  ...
  const buildHistoricalEventSamples = (
    metricSpecs,
    groupWeights,
    metricWeights,
    eventIdSet = null,
    ...
    eventMap.forEach((rows, eventId) => {
      ...
      if (samples.length >= 10) {
        ...
        excludedEvents.push({ eventId, sampleCount: samples.length });
      }
    });

    return { samplesByEvent, excludedEvents };
  };
  ...
  const crossValidateTopNLogisticByEvent = (eventSamples, lambdaCandidates, options = {}) => {
    ...
    const foldWeights = options?.foldWeights instanceof Map
      ? options.foldWeights
      : new Map();
    ...
    lambdas.forEach(l2 => {
      ...
      eventSamples.forEach((fold, idx) => {
        ...
        const weight = typeof foldWeights.get(String(fold.eventId)) === 'number'
          ? foldWeights.get(String(fold.eventId))
          : 1;
        totalLogLoss += evaluation.logLoss * weight;
        totalAccuracy += evaluation.accuracy * weight;
        foldsUsed += 1;
        foldWeightTotal += weight;
        foldDetails.push({
          eventId: fold.eventId,
          sampleCount: fold.samples.length,
          logLoss: evaluation.logLoss,
          accuracy: evaluation.accuracy,
          weight
        });
      ...
      if (foldsUsed > 0) {
        results.push({
          l2,
          avgLogLoss: totalLogLoss / Math.max(1, foldWeightTotal),
          avgAccuracy: totalAccuracy / Math.max(1, foldWeightTotal),
          foldsUsed,
          foldWeightTotal,
          foldDetails
        });
    ...
    const weightedEventCount = best?.foldDetails
      ? best.foldDetails.reduce((sum, fold) => sum + (typeof fold.weight === 'number' ? fold.weight : 1), 0)
      : eventSamples.length;
    ...
    return {
      ...
      weightedEventCount,
      effectiveEventCount: weightedEventCount,
      totalSamples: allSamples.length,
      bestL2: best.l2,
      avgLogLoss: best.avgLogLoss,
      avgAccuracy: best.avgAccuracy,
      foldsUsed: best.foldsUsed,
      eventIds: eventSamples.map(entry => entry.eventId),
      foldDetails: best.foldDetails || [],
      allSamples,
      finalModel
    };
  };

  const crossValidateTopNLogisticByEventKFold = (eventSamples, lambdaCandidates, options = {}) => {
    if (!Array.isArray(eventSamples) || eventSamples.length < 3) {
      return { success: false, message: 'Not enough events for CV', eventCount: eventSamples ? eventSamples.length : 0 };
    }
    ...
    const kRaw = parseInt(String(options?.k ?? '').trim(), 10);
    const requestedK = Number.isNaN(kRaw) ? null : kRaw;
    const foldCount = requestedK
      ? Math.max(2, Math.min(requestedK, eventSamples.length))
      : Math.min(5, eventSamples.length);

    if (!foldCount || foldCount < 2) {
      return { success: false, message: 'Invalid K for CV', eventCount: eventSamples.length };
    }
    ...
    const foldWeights = options?.foldWeights instanceof Map
      ? options.foldWeights
      : new Map();
    const seed = String(options?.seed || OPT_SEED_RAW || `${CURRENT_EVENT_ID}-cvkfold`).trim();
    const rng = createSeededRng(seed) || Math.random;
    const shuffled = [...eventSamples];
    ...
    lambdas.forEach(l2 => {
      ...
      folds.forEach((foldEntries, foldIndex) => {
        if (!foldEntries.length) return;
        const foldEventIds = foldEntries.map(entry => String(entry.eventId));
        const trainingSamples = folds
          .filter((_, idx) => idx !== foldIndex)
          .flatMap(group => group)
          .flatMap(entry => entry.samples);
        const testSamples = foldEntries.flatMap(entry => entry.samples);
        if (trainingSamples.length < 20 || testSamples.length < 10) return;
        const model = trainLogisticFromSamples(trainingSamples, { l2 });
        if (!model.success) return;
        const evaluation = evaluateLogisticModel(model, testSamples);
        const weight = foldEventIds.reduce((sum, eventId) => {
          const value = foldWeights.get(String(eventId));
        ...
        foldDetails.push({
          fold: foldIndex + 1,
          eventIds: foldEventIds,
          eventCount: foldEntries.length,
    ...
    const best = results[0];
    const finalModel = trainLogisticFromSamples(allSamples, { l2: best.l2 });
    const weightedEventCount = eventSamples.reduce((sum, entry) => {
      const value = foldWeights.get(String(entry.eventId));
      return sum + (typeof value === 'number' ? value : 1);
    }, 0);
  ...
  let currentGeneratedTop20CvKFoldSummary = null;
  let currentGeneratedGroupCorrelations = [];
  let currentGeneratedGroupTop20Correlations = [];
  let groupSignalSummary = null;
  const GROUP_SIGNAL_CORR_MIN = (() => {
    const raw = parseFloat(String(process.env.MODEL_GROUP_SIGNAL_CORR_MIN || '').trim());
    return Number.isNaN(raw) ? 0.2 : Math.max(0, raw);
  })();
  const GROUP_SIGNAL_CORR_FULL = (() => {
    const raw = parseFloat(String(process.env.MODEL_GROUP_SIGNAL_CORR_FULL || '').trim());
    return Number.isNaN(raw) ? 0.35 : Math.max(0.05, raw);
  })();
  const GROUP_SIGNAL_BLEND_MAX = (() => {
    const raw = parseFloat(String(process.env.MODEL_GROUP_SIGNAL_BLEND_MAX || '').trim());
    return Number.isNaN(raw) ? 0.6 : clamp01(raw);
  })();
  ...
  const STEP1C_APPROACH_ROUNDS_THRESHOLD = 500;
  const STEP1C_CV_APPROACH_CORR_MIN = 0.2;
  const STEP1C_PUTTING_LABELS = new Set(['SG Putting']);
  ...
  const isPuttingMetricLabel = label => {
    const normalized = String(label || '').toLowerCase();
    if (!normalized) return false;
    if (STEP1C_PUTTING_LABELS.has(label)) return true;
    return normalized.includes('putting') || normalized.includes('putt');
  };

  const STEP1C_MERGED_APPROACH_VALUE_MAP = new Map([
    ['Approach <150 FW GIR', ['Approach <150 FW GIR', 'Approach <150 Rough GIR']],
    ['Approach <150 FW SG', ['Approach <150 FW SG', 'Approach <150 Rough SG']],
    ['Approach <150 FW Prox', ['Approach <150 FW Prox', 'Approach <150 Rough Prox']],
    ['Approach <200 FW GIR', ['Approach <200 FW GIR', 'Approach >150 Rough GIR']],
    ['Approach <200 FW SG', ['Approach <200 FW SG', 'Approach >150 Rough SG']],
    ['Approach <200 FW Prox', ['Approach <200 FW Prox', 'Approach >150 Rough Prox']]
  ]);
  const STEP1C_MERGED_APPROACH_LABELS = new Set(STEP1C_MERGED_APPROACH_VALUE_MAP.keys());
  const STEP1C_LABEL_INDEX = new Map(GENERATED_METRIC_LABELS.map((label, idx) => [label, idx]));
  ...
  const buildMergedApproachValueGetter = label => {
    const sources = STEP1C_MERGED_APPROACH_VALUE_MAP.get(label);
    if (!sources || sources.length === 0) return null;
    return player => {
      if (!player || !Array.isArray(player.metrics)) return null;
      const values = sources.map(sourceLabel => {
        const idx = STEP1C_LABEL_INDEX.get(sourceLabel);
        if (typeof idx !== 'number') return null;
        return player.metrics[idx];
      });
      return averageFinite(values);
    };
  };

  const buildStep1cMetricLabels = ({ includeApproachBuckets = false, puttingOnly = false }) => {
    let labels = GENERATED_METRIC_LABELS.filter(label => !TRAINING_EXCLUDED_SET.has(label));
    if (puttingOnly) {
      labels = labels.filter(isPuttingMetricLabel);
    }
    if (!includeApproachBuckets) {
      labels = labels.filter(label => !APPROACH_BUCKET_LABELS.has(label));
    } else {
      labels = labels.filter(label => !APPROACH_ROUGH_BUCKET_LABELS.has(label));
    }
    return labels;
  };

  const buildStep1cMetricSpecs = ({ includeApproachBuckets = false, puttingOnly = false }) => {
    const labels = buildStep1cMetricLabels({ includeApproachBuckets, puttingOnly });
    const specs = labels
      .map(label => {
        const index = GENERATED_METRIC_LABELS.indexOf(label);
        const valueGetter = STEP1C_MERGED_APPROACH_LABELS.has(label)
          ? buildMergedApproachValueGetter(label)
          : null;
        return { label, index, valueGetter };
      })
      .filter(spec => typeof spec.index === 'number' && spec.index >= 0);
    return { labels, specs };
  };
  ...
  const resolveApproachDeltaRowsForEvent = (eventId, season = CURRENT_SEASON) => {
    ...
    const manifestForSeason = String(CURRENT_SEASON) === seasonValue
      ? (manifestEntries || [])
      : loadSeasonManifestEntries(DATA_ROOT_DIR, seasonValue);
    const eventEntry = resolveManifestEntryForEvent(manifestForSeason || [], {
      eventId,
      tournamentSlug: null,
      tournamentName: null
    });
    if (!eventEntry) return { rows: [], path: null };
    const deltaPath = findApproachDeltaFile(
      buildApproachDeltaSearchDirs(season, [OUTPUT_DIR, DATA_DIR, DEFAULT_DATA_DIR]),
      eventEntry.tournamentName,
      eventEntry.tournamentSlug,
      season
    );
  ...
  const resolveStep1cApproachRows = ({ eventId, season, allowApproach }) => {
    if (!allowApproach) {
    ...
    if (snapshotUsage.rows.length > 0) {
      return {
        rows: snapshotUsage.rows,
        source: snapshotUsage.meta?.source || 'snapshot_prior_year',
        path: snapshotUsage.meta?.path || null,
        reason: null
      };
    }
    return { rows: [], source: 'snapshot_missing', path: null, reason: 'snapshot_missing' };
  };

  const aggregateCorrelationLists = (acc, list, weight = 1) => {
    const safeWeight = typeof weight === 'number' && Number.isFinite(weight) ? weight : 1;
    (list || []).forEach(entry => {
      const label = entry?.label;
      if (!label) return;
      const samples = typeof entry.samples === 'number' ? entry.samples : 0;
      const correlation = typeof entry.correlation === 'number' ? entry.correlation : 0;
      const weightedSamples = samples * safeWeight;
      const existing = acc.get(label) || { sum: 0, samples: 0 };
      existing.sum += correlation * weightedSamples;
      existing.samples += weightedSamples;
      acc.set(label, existing);
    });
  };

  const finalizeAggregatedCorrelations = acc => Array.from(acc.entries()).map(([label, entry]) => {
    const index = GENERATED_METRIC_LABELS.indexOf(label);
    return {
      index: index >= 0 ? index : null,
      label,
      correlation: entry.samples > 0 ? entry.sum / entry.samples : 0,
      samples: entry.samples
    };
  });

  const groupRoundsByEventSeason = rounds => {
    const map = new Map();
    (rounds || []).forEach(row => {
      const eventId = String(row?.event_id || '').trim();
      const season = String(row?.year || row?.season || '').trim();
      if (!eventId || !season) return;
      const key = `${eventId}|${season}`;
      if (!map.has(key)) map.set(key, { eventId, season, rounds: [] });
      map.get(key).rounds.push(row);
    });
    return Array.from(map.values());
  };

  const computeStep1cCorrelationsForRounds = ({
    rounds,
    approachRows,
    metricSpecs,
    groupWeights,
    ...
    const results = buildResultsFromRows(rounds);
    if (!results.length) return { metric: [], top20: [] };
    const ranking = runRanking({
      roundsRawData: rounds,
      approachRawData: approachRows,
      groupWeights,
      metricWeights,
      includeCurrentEventRounds,
      fieldDataOverride
    ...
    return {
      metric: computeGeneratedMetricCorrelationsForLabels(ranking.players, results, metricSpecs),
      top20: computeGeneratedMetricTopNCorrelationsForLabels(ranking.players, results, metricSpecs, 20)
    };
  };

  const filterApproachRowsToField = (rows, fieldDataOverride) => {
    if (!Array.isArray(rows) || !Array.isArray(fieldDataOverride) || fieldDataOverride.length === 0) return rows || [];
    const fieldSet = new Set(
      fieldDataOverride
        .map(row => normalizeDgIdValue(row?.dg_id ?? row?.dgId))
        .filter(Boolean)
    );
    return (rows || []).filter(row => {
      const dgId = normalizeDgIdValue(row?.dg_id ?? row?.dgId);
      if (!dgId) return false;
      return fieldSet.has(dgId);
    });
  };

  const logStep1cMetricUsage = entries => {
    if (!Array.isArray(entries) || entries.length === 0) return;
    console.log('ℹ️  Step 1c metric scope detail:');
    entries.forEach(entry => {
      const scope = String(entry.scope || 'event').toUpperCase();
      const seasonLabel = entry.season ? `season ${entry.season}` : 'season n/a';
      const roundsLabel = typeof entry.rounds === 'number' ? entry.rounds : 0;
      const approachSource = entry.approach?.source || 'none';
      const approachPath = entry.approach?.path ? path.basename(entry.approach.path) : null;
      const approachReason = entry.approach?.reason ? ` (${entry.approach.reason})` : '';
      const approachRows = typeof entry.approach?.rowsCount === 'number' ? entry.approach.rowsCount : 0;
      const scopeWeight = typeof entry.scopeWeight === 'number' ? entry.scopeWeight.toFixed(2) : '1.00';
      console.log(`  - ${scope} event ${entry.eventId} (${seasonLabel}): rounds=${roundsLabel}, weight=${scopeWeight}, approach=${approachSource}${approachPath ? `:${approachPath}` : ''}, rows=${approachRows}${approachReason}`);
      if (Array.isArray(entry.metricLabels) && entry.metricLabels.length > 0) {
        console.log(`    Metrics (${entry.metricLabels.length}): ${entry.metricLabels.join(', ')}`);
      }
      if (Array.isArray(entry.metricSamples) && entry.metricSamples.length > 0) {
        const sampleText = entry.metricSamples
          .map(metric => `${metric.label}:${metric.samples ?? 0}`)
          .join(', ');
        console.log(`    Samples: ${sampleText}`);
      }
    });
  };

  const buildStep1cAggregatedCorrelations = ({
    rounds,
    templateConfig,
    similarCourseIds,
    puttingCourseIds,
    includeCurrentEventRounds,
    allowApproachBuckets,
    similarCourseBlend,
    puttingCourseBlend
  }) => {
    const baseSpec = buildStep1cMetricSpecs({ includeApproachBuckets: allowApproachBuckets, puttingOnly: false });
    const baseSpecNoApproach = buildStep1cMetricSpecs({ includeApproachBuckets: false, puttingOnly: false });
    const puttingSpec = buildStep1cMetricSpecs({ includeApproachBuckets: false, puttingOnly: true });
    const similarSet = new Set((similarCourseIds || []).map(value => String(value)));
    const puttingSet = new Set((puttingCourseIds || []).map(value => String(value)));
    const metricAcc = new Map();
    const top20Acc = new Map();
    const detailLogs = [];

    const groups = groupRoundsByEventSeason(rounds || []);
    groups.forEach(group => {
      const eventId = String(group.eventId || '').trim();
      if (!eventId) return;
      const season = String(group.season || '').trim();
      const isCurrentEvent = eventId === String(CURRENT_EVENT_ID);
      const isPutting = puttingSet.has(eventId);
      const isSimilar = similarSet.has(eventId);
      ...
      if (isCurrentEvent) {
        scopeEntries.push({
          scope: 'event',
          scopeWeight: 1,
          allowApproach: allowApproachBuckets
        });
      } else {
        if (isSimilar) {
          scopeEntries.push({
            scope: 'similar',
            scopeWeight: clamp01(similarCourseBlend, 0.3),
            allowApproach: allowApproachBuckets
          });
        }
        if (isPutting) {
          scopeEntries.push({
            scope: 'putting',
            scopeWeight: clamp01(puttingCourseBlend, 0.35),
            allowApproach: false
          });
        }
      ...
      scopeEntries.forEach(scopeEntry => {
        const isPuttingScope = scopeEntry.scope === 'putting';
        const approachInfo = isPuttingScope
          ? { rows: [], source: 'putting_only', path: null, reason: 'putting_only' }
          : resolveStep1cApproachRows({
              eventId,
              season,
              allowApproach: scopeEntry.allowApproach
            });
        const fieldDataOverride = buildFieldDataFromHistory(group.rounds, null);
        const approachRows = filterApproachRowsToField(approachInfo.rows || [], fieldDataOverride);
        const useApproachBuckets = scopeEntry.allowApproach && approachRows.length > 0;
        const metricSpecs = isPuttingScope
          ? puttingSpec.specs
          : (useApproachBuckets ? baseSpec.specs : baseSpecNoApproach.specs);
        const metricLabels = isPuttingScope
          ? puttingSpec.labels
          : (useApproachBuckets ? baseSpec.labels : baseSpecNoApproach.labels);
        const groupWeights = isPuttingScope
          ? removeApproachGroupWeights(templateConfig.groupWeights)
          : templateConfig.groupWeights;

        const correlations = computeStep1cCorrelationsForRounds({
          rounds: group.rounds,
          approachRows,
          metricSpecs,
          groupWeights,
          metricWeights: templateConfig.metricWeights,
          includeCurrentEventRounds,
          fieldDataOverride
        });

        aggregateCorrelationLists(metricAcc, correlations.metric, scopeEntry.scopeWeight);
        aggregateCorrelationLists(top20Acc, correlations.top20, scopeEntry.scopeWeight);
        ...
        detailLogs.push({
          ...
          scopeWeight: scopeEntry.scopeWeight,
          metricLabels,
          metricSamples: correlations.metric,
          approach: {
            source: approachInfo.source,
            path: approachInfo.path,
            reason: approachInfo.reason,
            rowsCount: approachRows.length
          }
        });
      });
    });
    ...
    return {
      metricCorrelations: finalizeAggregatedCorrelations(metricAcc),
      top20Correlations: finalizeAggregatedCorrelations(top20Acc),
      baseMetricLabels: baseSpec.labels,
      baseMetricSpecs: baseSpec.specs,
      puttingMetricLabels: puttingSpec.labels,
      puttingMetricSpecs: puttingSpec.specs,
      detailLogs
    };
  };

  const buildCvMetricLabels = ({ baseLabels, approachCorrelationList, allowApproachBuckets }) => {
    const approachCorrMap = new Map(
      (approachCorrelationList || []).map(entry => [entry.label, entry.correlation])
    );
    const labels = baseLabels.filter(label => {
      if (!APPROACH_BUCKET_LABELS.has(label)) return true;
      if (!allowApproachBuckets) return false;
      const corr = approachCorrMap.get(label);
      return typeof corr === 'number' && corr > STEP1C_CV_APPROACH_CORR_MIN;
    });
    if (!labels.some(isPuttingMetricLabel)) {
      labels.push('SG Putting');
    }
    return labels;
  };
  ...
  console.log('Correlate Step 1c metrics using event + similar + putting rounds with scoped metric sets and event-specific approach data.');
  console.log(`Contract: exclude SG Total, Greens in Regulation, Fairway Proximity, Rough Proximity, Birdie Chances Created; include SG Approach + Birdies or Better; approach buckets merged FW+Rough per distance and gated at ${STEP1C_APPROACH_ROUNDS_THRESHOLD}+ rounds (global gate; no per-event minimum); putting metrics always included.`);
  ...
  if (!HAS_CURRENT_RESULTS) {
    ...
    if (!currentTemplate) {
      ...
      const allowApproachBuckets = approachDataCurrent.length > 0
        && trainingRounds.length >= STEP1C_APPROACH_ROUNDS_THRESHOLD;
      const approachGateReason = allowApproachBuckets
        ? 'passed'
        : `blocked (rounds=${trainingRounds.length}, approachRows=${approachDataCurrent.length})`;
      const aggregated = buildStep1cAggregatedCorrelations({
      ...
      const metricLabelsForTraining = aggregated.baseMetricLabels;
      const metricSpecsForTraining = aggregated.baseMetricSpecs;
      TOP20_METRIC_LABELS = metricLabelsForTraining.slice();
      trainingMetricNotes = {
        included: metricLabelsForTraining,
        excluded: TRAINING_EXCLUDED_LABELS.slice(),
        derived: [
          'Birdies or Better = birdies + eagles (from historical rounds when available)',
          'Approach <150 FW/<200 FW metrics blended with rough buckets (FW+Rough per distance)'
        ]
      };
      ...
      const historicalGroupWeights = allowApproachBuckets
        ? currentTemplate.groupWeights
        : removeApproachGroupWeights(currentTemplate.groupWeights);
      ...
      if (trainingRounds.length > 0 && trainingResults.length > 0) {
        ...
        const historicalRanking = runRanking({
          ...
          approachRawData: allowApproachBuckets ? approachDataCurrent : [],
        ...
        const cvMetricLabels = buildCvMetricLabels({
          baseLabels: metricLabelsForTraining,
          approachCorrelationList: currentGeneratedMetricCorrelations,
          allowApproachBuckets
        });
        const cvMetricSpecs = cvMetricLabels
          .map(label => ({ label, index: GENERATED_METRIC_LABELS.indexOf(label) }))
          .filter(spec => spec.index >= 0);
        const eventSamplesPayload = buildHistoricalEventSamples(
          cvMetricSpecs,
          historicalGroupWeights,
          currentTemplate.metricWeights,
          eventIdSet,
          allowApproachBuckets ? approachDataCurrent : []
        );
        const eventSamples = eventSamplesPayload.samplesByEvent || [];
        const cvWeightEvent = getEnvNumberValue('MODEL_CV_EVENT_WEIGHT', 1);
        const cvWeightSimilar = getEnvNumberValue('MODEL_CV_SIMILAR_WEIGHT', 1);
        const cvWeightPutting = getEnvNumberValue('MODEL_CV_PUTTING_WEIGHT', 0.3);
        const cvWeightSimilarPutting = getEnvNumberValue('MODEL_CV_SIMILAR_PUTTING_WEIGHT', 1);
        const cvEventKRaw = parseInt(String(process.env.MODEL_CV_EVENT_K || '').trim(), 10);
        const cvEventK = Number.isNaN(cvEventKRaw) ? null : cvEventKRaw;
        const cvEventKSeedRaw = String(process.env.MODEL_CV_EVENT_K_SEED || '').trim();
        const cvEventKSeed = cvEventKSeedRaw || null;
        const eventIdStr = String(CURRENT_EVENT_ID);
        const similarIdSet = new Set(similarCourseIds.map(id => String(id)));
        const puttingIdSet = new Set(puttingCourseIds.map(id => String(id)));
        const foldWeights = new Map();
        const categoryByEventId = new Map();
        eventSamples.forEach(entry => {
          const eventId = String(entry.eventId);
          const isEvent = eventId === eventIdStr;
          const isSimilar = similarIdSet.has(eventId);
          const isPutting = puttingIdSet.has(eventId);
          const category = isEvent
            ? 'event'
            : (isSimilar && isPutting
              ? 'similar_putting'
              : (isSimilar ? 'similar' : (isPutting ? 'putting' : 'other')));
          categoryByEventId.set(eventId, category);
          let weight = 1;
          if (category === 'event') weight = cvWeightEvent;
          else if (category === 'similar_putting') weight = cvWeightSimilarPutting;
          else if (category === 'similar') weight = cvWeightSimilar;
          else if (category === 'putting') weight = cvWeightPutting;
          foldWeights.set(eventId, weight);
        });
        const filteredEventSamples = eventSamples.filter(entry => {
          const weight = foldWeights.get(String(entry.eventId));
          return !(typeof weight === 'number' && weight <= 0);
        });
        const excludedPuttingEvents = eventSamples
          .filter(entry => {
            const eventId = String(entry.eventId);
            return categoryByEventId.get(eventId) === 'putting' && (foldWeights.get(eventId) || 0) <= 0;
          })
          .map(entry => ({
            eventId: String(entry.eventId),
            sampleCount: Array.isArray(entry.samples) ? entry.samples.length : 0,
            reason: 'putting_only_scope'
          }));
        if (filteredEventSamples.length >= 3) {
          currentGeneratedTop20CvSummary = crossValidateTopNLogisticByEvent(filteredEventSamples, null, { foldWeights });
          if (currentGeneratedTop20CvSummary) {
            const existingExcluded = Array.isArray(eventSamplesPayload.excludedEvents)
              ? eventSamplesPayload.excludedEvents
              : [];
            const combinedExcluded = [...existingExcluded, ...excludedPuttingEvents];
            if (combinedExcluded.length) {
              currentGeneratedTop20CvSummary.excludedEvents = combinedExcluded;
            }
            if (excludedPuttingEvents.length) {
              currentGeneratedTop20CvSummary.excludedPuttingEvents = excludedPuttingEvents;
            }
            currentGeneratedTop20CvSummary.cvFoldCategories = Array.from(categoryByEventId.entries())
              .map(([eventId, category]) => ({ eventId, category, weight: foldWeights.get(eventId) }));
          }
          if (cvEventK && cvEventK > 1) {
            currentGeneratedTop20CvKFoldSummary = crossValidateTopNLogisticByEventKFold(
              filteredEventSamples,
              null,
              { foldWeights, k: cvEventK, seed: cvEventKSeed || OPT_SEED_RAW || `${CURRENT_EVENT_ID}-cvkfold` }
            );
          }
          ...
          currentGeneratedTop20CvSummary = {
            ...
            eventCount: filteredEventSamples.length,
            excludedEvents: [...(eventSamplesPayload.excludedEvents || []), ...excludedPuttingEvents],
            excludedPuttingEvents
          };
        }
        step1cUsedRounds = trainingRounds;

        currentGeneratedGroupCorrelations = computeGroupCorrelations(
          historicalRanking.players,
          trainingResults,
          metricConfig.groups
        );
        currentGeneratedGroupTop20Correlations = computeGroupCorrelations(
          historicalRanking.players,
          trainingResults,
          metricConfig.groups,
          20
      ...
      const metricDerivedGroupWeights = buildSuggestedGroupWeights(
        metricConfig,
        suggestedTop20MetricWeights
      );
      const groupSignalWeights = buildSuggestedGroupWeightsFromGroupSignals(
        currentGeneratedGroupTop20Correlations,
        'top20-group-signal'
      );
      const maxGroupAbs = currentGeneratedGroupTop20Correlations.reduce(
        (max, entry) => Math.max(max, Math.abs(entry?.correlation || 0)),
        0
      );
      const blendedGroupSignal = blendGroupSignalWeights({
        metricDerived: metricDerivedGroupWeights,
        groupSignal: groupSignalWeights,
        fallbackGroupWeights: metricConfig.groupWeights || currentTemplate.groupWeights || {},
        maxAbsCorrelation: maxGroupAbs,
        minCorr: GROUP_SIGNAL_CORR_MIN,
        fullCorr: GROUP_SIGNAL_CORR_FULL,
        maxShare: GROUP_SIGNAL_BLEND_MAX
      });
      suggestedTop20GroupWeights = {
        source: 'top20-group-blend',
        weights: blendedGroupSignal.weights
      };
      groupSignalSummary = {
        maxAbsCorrelation: maxGroupAbs,
        metricShare: blendedGroupSignal.metricShare,
        groupShare: blendedGroupSignal.groupShare,
        minCorr: GROUP_SIGNAL_CORR_MIN,
        fullCorr: GROUP_SIGNAL_CORR_FULL,
        maxShare: GROUP_SIGNAL_BLEND_MAX
      };
    ...
    if (!currentTemplate) {
      ...
      if (roundsForMetrics.length > 0) {
        const allowApproachBuckets = approachDataCurrent.length > 0
          && roundsForMetrics.length >= STEP1C_APPROACH_ROUNDS_THRESHOLD;
        const approachGateReason = allowApproachBuckets
          ? 'passed'
          : `blocked (rounds=${roundsForMetrics.length}, approachRows=${approachDataCurrent.length})`;
        const aggregated = buildStep1cAggregatedCorrelations({
          rounds: roundsForMetrics,
          templateConfig: currentTemplate,
          similarCourseIds,
          puttingCourseIds,
        ...
        TOP20_METRIC_LABELS = aggregated.baseMetricLabels.slice();
        const top20MetricSpecs = aggregated.baseMetricSpecs;
        trainingMetricNotes = {
        ...
        const cvMetricLabels = buildCvMetricLabels({
          baseLabels: TOP20_METRIC_LABELS,
          approachCorrelationList: currentGeneratedMetricCorrelations,
          allowApproachBuckets
        });
        const cvMetricSpecs = cvMetricLabels
          .map(label => ({ label, index: GENERATED_METRIC_LABELS.indexOf(label) }))
          .filter(spec => spec.index >= 0);
        const seasonEventSamples = buildEventSamplesBySeason(CURRENT_SEASON, cvMetricSpecs);
        let eventSamples = seasonEventSamples;
        let cvNote = null;
        if (seasonEventSamples.length < 3) {
          eventSamples = buildEventSamplesBySeason(null, cvMetricSpecs);
        ...
        if (eventSamples.length >= 3) {
          ...
          if (cvEventK && cvEventK > 1) {
            currentGeneratedTop20CvKFoldSummary = crossValidateTopNLogisticByEventKFold(
              eventSamples,
              null,
              { k: cvEventK, seed: cvEventKSeed || OPT_SEED_RAW || `${CURRENT_EVENT_ID}-cvkfold` }
            );
          }
          if (currentGeneratedTop20CvSummary.success && currentGeneratedTop20CvSummary.finalModel) {
            currentGeneratedTop20Logistic = summarizeLogisticModel(
      ...
      if (IS_POST_TOURNAMENT_RUN && currentTemplate) {
        ...
        if (top20BlendOutputDir) {
          ...
          try {
            ...
            const top20BlendArtifacts = new OutputArtifactManager({
              outputBaseName,
              tournamentSlug: blendSlug,
              tournamentName: TOURNAMENT_NAME || tournamentNameFallback,
            ...
            const outputPath = top20BlendArtifacts.buildPath({
  ...
  cvReliabilityKFold = currentGeneratedTop20CvKFoldSummary?.success
    ? computeCvReliability(currentGeneratedTop20CvKFoldSummary)
    : null;
  ...
  if (!HAS_CURRENT_RESULTS) {
    ...
    let blendedGroupWeights = blendGroupWeights(priorGroupWeights, filledGroupWeights, priorShare, modelShare);
    ...
    const approachCapMin = clamp01(getEnvNumberValue('MODEL_APPROACH_CAP_MIN', 0.25));
    const approachCapMax = clamp01(getEnvNumberValue('MODEL_APPROACH_CAP_MAX', 0.45));
    const approachCapCorr = typeof groupSignalSummary?.maxAbsCorrelation === 'number'
      ? groupSignalSummary.maxAbsCorrelation
      : 0;
    const approachCapNormalized = approachCapCorr <= GROUP_SIGNAL_CORR_MIN
      ? 0
    ...
    if (groupSignalSummary) {
      ...
      groupSignalSummary.approachCapMax = approachCapMax;
      groupSignalSummary.approachCapNormalized = approachCapNormalized;
    }
    ...
    let preEventDeltaScoresOverride = null;
    let preEventTrendScores = null;
    let preEventPredictiveScores = null;
    if (IS_PRE_TOURNAMENT_RUN && !HAS_CURRENT_RESULTS) {
      const rollingWeightRaw = getEnvNumberValue('MODEL_APPROACH_DELTA_TREND_ROLLING_WEIGHT', 0.8);
      const priorWeightRaw = getEnvNumberValue('MODEL_APPROACH_DELTA_TREND_PRIOR_WEIGHT', 0.2);
      const weightTotal = rollingWeightRaw + priorWeightRaw;
      const rollingWeight = weightTotal > 0 ? rollingWeightRaw / weightTotal : 0.8;
      const priorWeight = weightTotal > 0 ? priorWeightRaw / weightTotal : 0.2;

      const rollingRows = Array.isArray(approachDeltaRollingRows) ? approachDeltaRollingRows : null;
      const priorRows = Array.isArray(approachDeltaPriorBlendRows) ? approachDeltaPriorBlendRows : null;
      const hasRolling = rollingRows && rollingRows.length > 0;
      const hasPrior = priorRows && priorRows.length > 0;

      if (hasRolling || hasPrior) {
        const rollingAlignment = hasRolling
          ? buildApproachDeltaAlignmentFromRollingRows(approachDeltaMetricSpecs, rollingRows)
          : new Map();
        const priorAlignment = hasPrior
          ? buildApproachDeltaAlignmentFromRollingRows(approachDeltaMetricSpecs, priorRows)
          : new Map();

        const rollingTrendScores = (hasRolling && rollingAlignment.size > 0)
          ? buildApproachDeltaPlayerScores(approachDeltaMetricSpecs, rollingRows, rollingAlignment)
          : [];
        const priorTrendScores = (hasPrior && priorAlignment.size > 0)
          ? buildApproachDeltaPlayerScores(approachDeltaMetricSpecs, priorRows, priorAlignment)
          : [];

        preEventTrendScores = blendApproachDeltaTrendScores({
          rollingScores: rollingTrendScores,
          priorScores: priorTrendScores,
          rollingWeight,
          priorWeight
        });

        const predictiveRows = hasRolling ? rollingRows : (hasPrior ? priorRows : []);
        if (predictiveRows.length > 0) {
          ...
          preEventPredictiveScores = buildApproachDeltaPlayerScores(
            approachDeltaMetricSpecs,
            predictiveRows,
            predictiveAlignmentMap
          );
        ...
        const derivedDeltaScores = buildDeltaScoresByIdFromScores(
          preEventTrendScores || [],
          preEventPredictiveScores || []
        );
        if (derivedDeltaScores && Object.keys(derivedDeltaScores).length > 0) {
          preEventDeltaScoresOverride = derivedDeltaScores;
        }

        approachDeltaTrendBlendSummary = {
          mode: hasRolling && hasPrior ? 'rolling_blend' : (hasRolling ? 'rolling_only' : 'prior_only'),
          rollingWeight,
          priorWeight,
          rollingRows: hasRolling ? rollingRows.length : 0,
          priorRows: hasPrior ? priorRows.length : 0,
          rollingFiles: approachDeltaRollingFiles,
          priorPath: approachDeltaPriorBlendPath
        };
        approachDeltaPriorMeta = {
          ...(approachDeltaPriorMeta || {}),
          blend: approachDeltaTrendBlendSummary
        };
      }
    }
    ...
    const preEventRanking = runRanking({
      ...
      useDeltaScores: true,
      deltaScoresOverride: preEventDeltaScoresOverride
    });
    ...
    const preEventMetricDiagnostics = buildMetricStatsDiagnostics(preEventRanking.groupStats, {
      stdDevThreshold: 0.05,
      minCount: 10
    });
    if (approachDeltaAlignmentMap.size > 0 && approachDeltaRows.length > 0) {
      const trendScores = preEventTrendScores || buildApproachDeltaPlayerScores(
        approachDeltaMetricSpecs,
        approachDeltaRows,
        approachDeltaAlignmentMap
      );
      ...
      const predictiveScores = preEventPredictiveScores || buildApproachDeltaPlayerScores(
        approachDeltaMetricSpecs,
        approachDeltaRows,
        predictiveAlignmentMap
      );
    ...
    const preEventArtifacts = new OutputArtifactManager({
      outputBaseName,
    ...
    preEventRankingPath = preEventArtifacts.buildPath({
      artifactType: OUTPUT_ARTIFACTS.PRE_EVENT_RANKINGS_JSON
    });
    preEventRankingCsvPath = preEventArtifacts.buildPath({
    ...
    const output = {
      ...
      metricStatsDiagnostics: preEventMetricDiagnostics,
      lowDataAlert,
      ...
      currentGeneratedTop20Correlations,
      currentGeneratedGroupCorrelations,
      currentGeneratedGroupTop20Correlations,
      currentGeneratedTop20Logistic,
      currentGeneratedTop20CvSummary,
    ...
    const outputPath = preEventArtifacts.buildPath({
    ...
    textLines.push('');
    textLines.push('MODEL RELIABILITY CHECK:');
    textLines.push(`  Field integrity: ${fieldIntegritySummary.status || 'ok'} (field=${fieldIntegritySummary.fieldPlayers}, approachOverlap=${(fieldIntegritySummary.approachOverlap * 100).toFixed(1)}%)`);
    ...
    if (preEventMetricDiagnostics.flagged.length === 0) {
      ...
      textLines.push(`  Metric stability flags: ${preEventMetricDiagnostics.flagged.length}`);
      preEventMetricDiagnostics.flagged.slice(0, 10).forEach(entry => {
        const reasons = Array.isArray(entry.reasons) ? entry.reasons.join(', ') : 'n/a';
        textLines.push(`  - ${entry.group}::${entry.metric} (reasons: ${reasons})`);
        textLines.push('    Suggestion: review in modelCore (down-weight, cap, or remove if persistently unstable).');
      });
    }
    textLines.push('');
    ...
    if (approachDeltaPriorMode === 'rolling_average' || approachDeltaPriorMode === 'fallback_average') {
      ...
      if (IS_PRE_TOURNAMENT_RUN) {
        textLines.push('  Note: Pre-event runs do not compute approach delta correlations because results are not available yet.');
      }
    } else {
    ...
    textLines.push('');
    textLines.push('GROUP TOP-20 SIGNAL (composite groups):');
    if (!currentGeneratedGroupTop20Correlations.length) {
      textLines.push('  No group correlations computed.');
    } else {
      const groupTop = [...currentGeneratedGroupTop20Correlations]
        .sort((a, b) => Math.abs(b.correlation) - Math.abs(a.correlation))
        .slice(0, 10);
      groupTop.forEach(entry => {
        textLines.push(`  ${entry.groupName}: Corr=${entry.correlation.toFixed(4)}, Samples=${entry.samples}`);
      });
    }
    if (groupSignalSummary) {
      textLines.push(`  Group signal blend share: ${(groupSignalSummary.groupShare * 100).toFixed(1)}% (max|corr|=${groupSignalSummary.maxAbsCorrelation.toFixed(3)}, min gate=${groupSignalSummary.minCorr}, full=${groupSignalSummary.fullCorr}).`);
    }
    textLines.push('');
    ...
    textLines.push('');
    textLines.push('ACTIONABLE RECOMMENDATIONS:');
    textLines.push(`  Update template ${preEventTemplateName} in utilities/weightTemplates.js with the blended weights below.`);
    const groupChanges = summarizeWeightChanges(priorGroupWeights, blendedGroupWeights, 8);
    if (groupChanges.length === 0) {
      textLines.push('  No material group-weight changes detected.');
    } else {
      groupChanges.forEach(change => {
        textLines.push(`  - groupWeights.${change.key}: ${change.from.toFixed(4)} -> ${change.to.toFixed(4)} (Δ=${change.delta.toFixed(4)})`);
      });
    }
    const metricChanges = summarizeMetricWeightChanges(priorMetricWeights, adjustedMetricWeights, 12);
    if (metricChanges.length === 0) {
      textLines.push('  No material metric-weight changes detected.');
    } else {
      metricChanges.forEach(change => {
        textLines.push(`  - metricWeights.${change.key}: ${change.from.toFixed(4)} -> ${change.to.toFixed(4)} (Δ=${change.delta.toFixed(4)})`);
      });
    }
    textLines.push(`  CV gate: include approach buckets only if Corr > ${STEP1C_CV_APPROACH_CORR_MIN}.`);
    textLines.push('');
    ...
    const textOutputPath = preEventArtifacts.buildPath({
    ...
    if (WRITE_TEMPLATES || DRY_RUN) {
      ...
      if (HAS_CURRENT_RESULTS) {
        ...
        console.log('ℹ️  Pre-event run: delta player scores not written (rankings must use snapshot-only data).');
      }
    }
  ...
  if (!TEMPLATE && preferredTemplateName) {
    ...
    if (preferredResult) {
      ...
      optimizationTemplate = preferredResult;
      console.log(`ℹ️  Using course-context template: ${preferredTemplateName} (baseline + optimization seed)`);
    } else {
  ...
  const resolveEventCourseTypeKey = eventId => {
    ...
    let entry = null;
    if (courseContextInitial && typeof courseContextInitial === 'object') {
      if (courseContextInitial.byEventId && courseContextInitial.byEventId[eventKey]) {
        entry = courseContextInitial.byEventId[eventKey];
      } else if (courseContextInitial[eventKey]) {
        entry = courseContextInitial[eventKey];
      }
    }
    if (!entry) {
      entry = resolveCourseContextEntry(courseContextInitial, { eventId: eventKey });
    }
  ...
  const postEventArtifacts = new OutputArtifactManager({
    outputBaseName,
    tournamentSlug: canonicalTournamentSlug || normalizeTournamentSlug(TOURNAMENT_NAME || tournamentNameFallback),
    tournamentName: TOURNAMENT_NAME || tournamentNameFallback,
    modeRoot: OUTPUT_DIR
  ...
  let tournamentResultsSnapshotMeta = null;
  if (IS_POST_TOURNAMENT_RUN && HAS_CURRENT_RESULTS) {
    const rankingCsvCandidates = [
      preEventOutputDir ? path.resolve(preEventOutputDir, `${outputBaseName}_pre_event_rankings.csv`) : null,
      postEventOutputDir ? path.resolve(postEventOutputDir, `${outputBaseName}_pre_event_rankings.csv`) : null,
      preEventOutputDir ? path.resolve(preEventOutputDir, `${outputBaseName}_pre_event_rankings.csv`.toLowerCase()) : null,
    ...
    const rankingCsvPathForResults = findFirstExisting(rankingCsvCandidates);

    let modelById = new Map();
    if (rankingCsvPathForResults) {
      const resultMetricSpecs = resultsCsvUtils.buildResultsMetricSpecs();
      const csvModelById = resultsCsvUtils.parseRankingCsvModelValues(rankingCsvPathForResults, resultMetricSpecs);
      csvModelById.forEach((entry, dgId) => modelById.set(dgId, entry));
    }
    if (modelById.size === 0 && optimizedRankingCurrent?.players?.length) {
      modelById = buildModelByIdFromRankingPlayers(optimizedRankingCurrent.players);
    }

    const actualMetricsById = resultsCsvUtils.buildActualMetricsFromHistory(historyData, CURRENT_EVENT_ID, effectiveSeason);
    const actualTrendsById = resultsCsvUtils.buildActualTrendsFromHistory(historyData, CURRENT_EVENT_ID, effectiveSeason);
    const resultsRows = buildTournamentResultsSnapshotRows({
      results: resultsCurrent,
      modelById,
      actualMetricsById
    });
    const metricStats = computeSnapshotMetricStats(resultsRows);
    const zScores = buildSnapshotZScores(resultsRows, metricStats);

    const resultsSlug = canonicalTournamentSlug
      || normalizeTournamentSlug(TOURNAMENT_NAME || tournamentNameFallback)
      || outputBaseName;
    const resultsJsonPath = postEventArtifacts.buildPath({
      artifactType: OUTPUT_ARTIFACTS.TOURNAMENT_RESULTS_JSON,
      tournamentSlug: resultsSlug,
      tournamentName: TOURNAMENT_NAME || tournamentNameFallback
    });
    const resultsCsvPath = postEventArtifacts.buildPath({
      artifactType: OUTPUT_ARTIFACTS.TOURNAMENT_RESULTS_CSV,
      tournamentSlug: resultsSlug,
      tournamentName: TOURNAMENT_NAME || tournamentNameFallback
    });

    let resultsSheetCsv = null;
    if (resultsRows.length > 0) {
      const rankingsById = modelById;
      const resultsById = new Map(
        resultsCurrent
          .filter(entry => entry && entry.dgId)
          .map(entry => [String(entry.dgId).trim(), entry.finishPosition])
      );
      const approachDeltaById = resultsCsvUtils.buildApproachDeltaMap(Array.isArray(approachDeltaRows) ? approachDeltaRows : []);
      resultsSheetCsv = resultsCsvUtils.buildPostEventResultsCsv({
        rankingsById,
        resultsById,
        actualMetricsById,
        actualTrendsById,
      ...
      fs.writeFileSync(resultsCsvPath, resultsSheetCsv);
    }
    ...
    const resultsPayload = {
      ...
      metricStats,
      zScores,
      resultsSheetCsv: resultsSheetCsv || undefined,
      sources: {
        rankingsCsv: rankingCsvPathForResults || null
      }
    };

    fs.writeFileSync(resultsJsonPath, JSON.stringify(resultsPayload, null, 2));
    tournamentResultsSnapshotMeta = {
      path: resultsJsonPath,
      csvPath: resultsSheetCsv ? resultsCsvPath : null,
      rankingsCsv: rankingCsvPathForResults || null,
      totalRows: resultsRows.length
    };
  }

  output.tournamentResultsSnapshot = tournamentResultsSnapshotMeta;

  const outputPath = postEventArtifacts.buildPath({
  ...
  if (currentGeneratedTop20CvKFoldSummary && currentGeneratedTop20CvKFoldSummary.success) {
    textLines.push('  K-FOLD SUMMARY:');
    textLines.push(`    K: ${currentGeneratedTop20CvKFoldSummary.k}`);
    textLines.push(`    Events: ${currentGeneratedTop20CvKFoldSummary.eventCount}`);
    textLines.push(`    Samples: ${currentGeneratedTop20CvKFoldSummary.totalSamples}`);
    textLines.push(`    Best L2: ${currentGeneratedTop20CvKFoldSummary.bestL2}`);
    textLines.push(`    Avg LogLoss: ${currentGeneratedTop20CvKFoldSummary.avgLogLoss.toFixed(4)}`);
    textLines.push(`    Avg Accuracy: ${(currentGeneratedTop20CvKFoldSummary.avgAccuracy * 100).toFixed(1)}%`);
    textLines.push(`    Folds Used: ${currentGeneratedTop20CvKFoldSummary.foldsUsed}`);
  }
  textLines.push('');
  textLines.push(`SUGGESTED METRIC WEIGHTS (TOP-20) - SOURCE: ${suggestedTop20MetricWeights.source}`);
  if (!suggestedTop20MetricWeights.weights.length) {
  ...
  textLines.push('');
  const textOutputPath = postEventArtifacts.buildPath({
  ...
  if (!runContext.isResultsOnly) {
    const shouldAutoRunValidation = runContext.isPost
      && !runContext.isResultsOnly
      && !runContext.isValidationOnly;
    const validationSlug = normalizeTournamentSlug(TOURNAMENT_NAME || tournamentNameFallback) || baseName;
    const overrideGate = runContext.validationRequested;
    const hasExistingValidation = (() => {
      if (!validationOutputsDir) return false;
      const classificationPath = path.resolve(
        validationOutputsDir,
        `${OUTPUT_NAMES.courseTypeClassification}.json`
      );
      const classificationPayload = readJsonFile(classificationPath);
      const entries = Array.isArray(classificationPayload?.entries) ? classificationPayload.entries : [];
      if (!entries.length) return false;
      const eventKey = String(CURRENT_EVENT_ID || '').trim();
      const normalizedSlug = validationSlug;
      return entries.some(entry => {
        const entryEventId = entry?.eventId !== undefined && entry?.eventId !== null
          ? String(entry.eventId).trim()
          : '';
        const entrySlug = normalizeTournamentSlug(entry?.tournament || '');
        return (eventKey && entryEventId && entryEventId === eventKey)
          || (normalizedSlug && entrySlug && entrySlug === normalizedSlug);
      });
    })();

    if (shouldAutoRunValidation && hasExistingValidation && !overrideGate) {
      const reasonText = 'outputs already exist for this tournament';
      console.log(`ℹ️  Validation skipped: ${reasonText}.`);
      validationRunStatus = { status: 'skipped', reason: reasonText };
    } else if (shouldAutoRunValidation) {
      try {
        const { runValidation, runSeasonValidation } = require('../utilities/validationRunner');
        ...
        validationRunStatus = {
          status: 'ran',
          reason: overrideGate ? 'gate override' : null
        };

        if (typeof runValidation === 'function') {
          const overrideNote = overrideGate ? ' (gate override)' : '';
          console.log(`🔄 Running validation pipeline (post-tournament, slug=${validationSlug})${overrideNote}...`);
          const validationResult = await runValidation({
            season: CURRENT_SEASON,
            dataRootDir: DATA_ROOT_DIR,
            tournamentName: TOURNAMENT_NAME,
        ...
        if (typeof runSeasonValidation === 'function') {
          const overrideNote = overrideGate ? ' (gate override)' : '';
          console.log(`🔄 Running validation season rollup${overrideNote}...`);
          const seasonResult = await runSeasonValidation({
            season: CURRENT_SEASON,
            dataRootDir: DATA_ROOT_DIR,
            logger: console,
        ...
        console.warn(`⚠️  Validation pipeline failed: ${error.message}`);
        validationRunStatus = { status: 'failed', reason: error.message };
      }
    }
  }

  if (validationRunStatus) {
    humanSummaryLines.push('');
    humanSummaryLines.push('Validation orchestration');
    if (validationRunStatus.status === 'ran') {
      const reasonSuffix = validationRunStatus.reason
        ? ` (${validationRunStatus.reason})`
        : '';
      humanSummaryLines.push(`  - Auto validation: ran (tournament + season rollup)${reasonSuffix}`);
    } else if (validationRunStatus.status === 'skipped') {
      humanSummaryLines.push(`  - Auto validation: skipped (${validationRunStatus.reason})`);
    } else if (validationRunStatus.status === 'failed') {
      humanSummaryLines.push(`  - Auto validation: failed (${validationRunStatus.reason})`);
    }
  }
